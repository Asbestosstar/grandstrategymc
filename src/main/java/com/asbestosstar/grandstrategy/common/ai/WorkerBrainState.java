 ...
 *
 * This object deliberately contains only primitive/immutable-style data so it can be
 * saved with the world and copied safely into background planner requests. Minecraft
 * entities, levels, chunks, navigators and block states must never be stored here.
 */
public final class WorkerBrainState {
    public String intent = WorkerIntent.IDLE.name();

    /** Final objective. Intermediate Minecraft navigation targets are never written here. */
    ...
    /** Small bounded memory of bad locations so replans do not repeat the same mistake. */
    ...
    public void rememberFailure(int x, int y, int z, NavigationFailure failure, long tick) {
        ...
        while (failedLocations.size() > 16) failedLocations.remove(0);
    }
