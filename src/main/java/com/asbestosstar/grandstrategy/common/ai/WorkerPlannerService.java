 ...
 *
 * The service receives immutable primitive snapshots only. It never accesses a live
 * Minecraft Level, Entity, Chunk, PathNavigation or BlockState from a background
 * thread. Results are polled and applied later by PhysicalVillagerSystem on the
 * authoritative server tick thread.
 */
public final class WorkerPlannerService {
    ...
    public static WorkerPlannerService getInstance() { return INSTANCE; }
    ...
    public synchronized void stop() {
        for (CompletableFuture<PlannerResult> future : pending.values()) future.cancel(true);
        pending.clear();
        if (pool != null) {
            pool.shutdownNow();
            try {
    ...
    private PlannerResult planSurface(String workerUuid, long generation, NavigationSnapshot s) {
        ...
        for (int idx : s.penalisedCells()) if (idx >= 0 && idx < n) penalised[idx] = true;
        ...
        while (!open.isEmpty()) {
            ...
            for (int d = 0; d < dx.length; d++) {
                int nx = cx + dx[d];
                int nz = cz + dz[d];
                if (!s.inside(nx, nz)) continue;
                int ni = s.index(nx, nz);
                byte terrain = s.terrain()[ni];
                if (terrain == NavigationSnapshot.BLOCKED || closed[ni]) continue;
                ...
                if (d >= 4) base *= 1.41421356237;
                base += rise * 0.42;
                if (penalised[ni]) base += 18.0;
        ...
        if (waypoints.isEmpty()) {
            waypoints = List.of(new WorkerBrainState.BrainWaypoint(
                    s.intendedGoalX(), s.intendedGoalY(), s.intendedGoalZ()));
        }
        return new PlannerResult(workerUuid, generation, false, true,
                NavigationFailure.NONE, List.copyOf(waypoints));
    }
    ...
    private PlannerResult planEscape(String workerUuid, long generation, EscapeSnapshot s) {
        ...
        int sy = s.startY();
        int sz = s.startZ();
        if (!s.inside(sx, sy, sz)) return failed(workerUuid, generation, true, NavigationFailure.UNKNOWN);

        int[] goal = chooseEscapeGoal(s);
        if (goal == null) return failed(workerUuid, generation, true, NavigationFailure.PATH_NOT_FOUND);
        ...
        while (!open.isEmpty()) {
            ...
            for (int[] d : dirs) {
                ...
                for (int dy = 1; dy >= -1; dy--) {
                    int ny = cy + dy;
                    if (!s.inside(nx, ny, nz) || ny <= 0 || ny + 1 >= s.height()) continue;
        ...
        for (int i = 1; i < reverse.size(); i++) {
            ...
            int z = yz % s.depth();
            int y = yz / s.depth();
            waypoints.add(new WorkerBrainState.BrainWaypoint(
                    s.originX() + x, s.originY() + y, s.originZ() + z));
        }
        return new PlannerResult(workerUuid, generation, true, true,
                NavigationFailure.NONE, List.copyOf(waypoints));
    }
    ...
    private double escapePositionCost(EscapeSnapshot s, int x, int y, int z, int dy) {
        ...
        byte head = s.voxel(x, y + 1, z);
        byte below = s.voxel(x, y - 1, z);
        if (feet == EscapeSnapshot.PROTECTED || head == EscapeSnapshot.PROTECTED) return Double.POSITIVE_INFINITY;
        if (below == EscapeSnapshot.AIR || below == EscapeSnapshot.WATER) return Double.POSITIVE_INFINITY;
        double cost = 1.0 + Math.max(0, dy) * 0.5 + Math.max(0, -dy) * 0.2;
        cost += voxelBreakCost(feet) + voxelBreakCost(head);
        if (feet == EscapeSnapshot.WATER || head == EscapeSnapshot.WATER) cost += 3.0;
        return cost;
    }
     ...
     * submit a real stream scan/count; otherwise this is a zero-allocation CPU touch.
     * Route graph traversal remains on ordinary CPU cores because it is branch-heavy.
     */
    private void daxFriendlyTouch(byte[] data) {
        ...
        if (checksum == Integer.MIN_VALUE) throw new AssertionError();
    }
