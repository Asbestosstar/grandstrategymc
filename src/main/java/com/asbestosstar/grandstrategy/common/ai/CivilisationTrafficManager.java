...
/**
 * Server-thread shared memory for recurring movement bottlenecks. Several workers
 * failing at the same coarse location turn an individual path problem into a
 * civilisation infrastructure request that road builders can service once for all.
 */
public final class CivilisationTrafficManager {
    ...
    public static CivilisationTrafficManager getInstance() { return INSTANCE; }

    public synchronized void clear() { hotspots.clear(); }
    ...
    public synchronized InfrastructureNeed recordFailure(String civilisationId, int x, int y, int z,
        ...
        if (civilisationId == null || civilisationId.isBlank()) return null;
        prune(tick);
    ...
    public synchronized void noteSuccess(String civilisationId, int x, int z, long tick) {
        ...
        if (hotspot != null) {
            ...
            hotspot.lastTick = tick;
            if (hotspot.score == 0) hotspots.remove(civilisationId + ':' + cx + ':' + cz);
        }
    }
