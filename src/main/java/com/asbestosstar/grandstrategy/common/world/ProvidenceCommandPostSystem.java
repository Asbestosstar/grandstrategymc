...
public final class ProvidenceCommandPostSystem {
	...
	/**
	 * Called on the Minecraft server thread after ProvidenceSystem has updated
	 * geography.
	 */
	public static void tick(MinecraftServer server) {
		if (server == null)
			return;
		ticks++;
		if (ticks % RECONCILE_INTERVAL_TICKS != 0)
			return;
	...
	public static boolean captureByAdministrator(ServerLevel level, Providence providence, String civilisationId) {
		if (level == null || providence == null || civilisationId == null || providence.getCity() == null)
			return false;
		if (providence.territoryControlShare(civilisationId) <= ProvidenceSystem.ADMIN_CAPTURE_THRESHOLD)
			return false;
		String previous = providence.getCity().getControllerId();
		if (previous != null && WarSystem.getInstance().areAtWar(civilisationId, previous))
			return false;
	...
	public static boolean breakAndCaptureBySoldier(ServerLevel level, Providence providence, String civilisationId) {
		...
		if (previous == null || Objects.equals(previous, civilisationId)
				|| !WarSystem.getInstance().areAtWar(civilisationId, previous))
			return false;
