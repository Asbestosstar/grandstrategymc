 ...
 *
 * A dedicated-server client must never read the client's local DataManager as
 * if it were the server world. All GUI screens read this mirror instead.
 */
public final class StrategyClientState {
	...
	synchronized void apply(NetworkManager.SyncState state) {
		...
		if (state.civilisations != null) {
			for (Civilisation civilisation : state.civilisations) {
				if (civilisation == null || civilisation.getId() == null)
					continue;
		...
		if (state.providences != null) {
			for (Providence providence : state.providences) {
				if (providence == null || providence.getId() == null)
					continue;
		...
		if (state.focusTrees != null) {
			for (FocusTree tree : state.focusTrees) {
				if (tree == null || tree.getCivilisationId() == null)
					continue;
		...
		if (state.technologies != null)
			for (Technology value : state.technologies)
				if (value != null && value.getId() != null)
					nextTechnologies.put(value.getId(), value);
		Map<String, FactoryType> nextFactoryTypes = new HashMap<>();
		if (state.factoryTypes != null)
			for (FactoryType value : state.factoryTypes)
				if (value != null && value.getId() != null)
					nextFactoryTypes.put(value.getId(), value);
		Map<String, FactoryRecipe> nextFactoryRecipes = new HashMap<>();
		if (state.factoryRecipes != null)
			for (FactoryRecipe value : state.factoryRecipes)
				if (value != null && value.getId() != null)
					nextFactoryRecipes.put(value.getId(), value);
		Map<String, Religion> nextReligions = new HashMap<>();
		if (state.religions != null)
			for (Religion value : state.religions)
				if (value != null && value.getId() != null)
					nextReligions.put(value.getId(), value);
		Map<String, Ideology> nextIdeologies = new HashMap<>();
		if (state.ideologies != null)
			for (Ideology value : state.ideologies)
				if (value != null && value.getId() != null)
					nextIdeologies.put(value.getId(), value);
		Map<String, Leader> nextLeaders = new HashMap<>();
		if (state.leaders != null)
			for (Leader value : state.leaders) {
				if (value == null || value.getId() == null)
					continue;
		...
		if (!state.fullMap && previousMap != null) {
			for (WorldMapTracker.MapTile tile : previousMap.tiles()) {
				if (tile != null)
					mergedTiles.put(WorldMapTracker.chunkKey(tile.chunkX(), tile.chunkZ()), tile);
			}
		}
		if (state.tiles != null) {
		...
		this.mapSnapshot = new WorldMapTracker.Snapshot(Collections.unmodifiableList(tiles),
				Collections.unmodifiableSet(keys), state.projectionOriginSet, state.projectionOriginBlockX,
				state.projectionOriginBlockZ, state.minChunkX, state.maxChunkX, state.minChunkZ, state.maxChunkZ);
	...
	public Map<String, FocusTree> getFocusTrees() {
		return focusTrees;
	}

	public Map<String, GrandStrategyEvent> getEvents() {
		return events;
	}
	...
	public Map<String, FactoryType> getFactoryTypes() {
		return factoryTypes;
	}

	public Map<String, FactoryRecipe> getFactoryRecipes() {
	...
	public Map<String, Leader> getLeaders() {
		return leaders;
	}

	public Leader getLeader(String id) {
		return id == null ? null : leaders.get(id);
	}

	public List<WarSystem.WarState> getWars() {
		return wars;
	}

	public List<PhysicalVillagerSystem.VillagerMapMarker> getVillagers() {
		return villagers;
	}

	public List<PhysicalVillagerSystem.WorkZoneMapMarker> getWorkZones() {
		return workZones;
	}
