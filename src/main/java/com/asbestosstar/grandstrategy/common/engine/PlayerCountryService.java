...
public final class PlayerCountryService {
	...
	public static Civilisation findForIgn(String ign) {
		...
		for (Civilisation civilisation : DataManager.getCivilisations().values()) {
			if (isLivePlayerCountry(civilisation) && civilisation.getFounderIgn() != null
	...
	/**
	 * Preferred multiplayer ownership lookup. Old saves without UUIDs fall back to
	 * IGN.
	 */
	public static Civilisation findForPlayer(String uuid, String ign) {
		if (uuid != null && !uuid.isBlank()) {
			for (Civilisation civilisation : DataManager.getCivilisations().values()) {
				if (isLivePlayerCountry(civilisation) && uuid.equalsIgnoreCase(civilisation.getFounderUuid())) {
					return civilisation;
				}
			}
		}
	...
	private static boolean isLivePlayerCountry(Civilisation civilisation) {
		return civilisation != null && civilisation.isPlayerCreated() && civilisation.isActive()
	...
	/** Backwards-compatible overload. */
	public static synchronized Civilisation createCountry(String ign, long currentYear, Integer playerBlockX,
	 ...
	 * name.
	 */
	public static synchronized Civilisation createCountry(String ign, String uuid, long currentYear,
			Integer playerBlockX, Integer playerBlockZ) {
		...
		if (playerBlockX != null && playerBlockZ != null) {
			...
			if (ProvidenceSystem.isAvailableForNewCountry(containing))
				homeland = containing;
		}
		...
		String id = baseId;
		int suffix = 2;
		while (DataManager.getCivilisations().containsKey(id))
			id = baseId + "_" + suffix++;
		String leaderId = id + "_founder";

		Civilisation civilisation = new Civilisation(id, safeIgn, leaderId, "nonaligned", "secular",
				List.of(homeland.getId()), currentYear, 50, 50);
		civilisation.setDefaultCityNames(
				List.of(safeIgn, safeIgn + " City", safeIgn + " Port", safeIgn + " Heights", safeIgn + " Junction"));
		civilisation.initialisePlayerCountry(safeIgn, uuid, currentYear);
		if (playerBlockX != null && playerBlockZ != null) {
			civilisation.setWorldMapPosition(playerBlockX, playerBlockZ);
		}

		DataManager.getLeaders().put(leaderId, new Leader(leaderId, safeIgn, false, List.of("Founder")));
		...
		System.out.println("Created player country " + civilisation.getName() + " [" + civilisation.getId() + "] with "
				+ civilisation.getPopulation() + " villagers by colonising existing providence " + homeland.getId()
				+ " and city " + homeland.getCity().getName() + ".");
		return civilisation;
	}
	...
	public static boolean improveRelations(String sourceId, String targetId) {
		...
		if (source == null || target == null || source == target || !source.isActive() || !target.isActive())
			return false;
		if (!source.spendPoliticalPower(10.0))
			return false;
	...
	public static boolean declareWar(String sourceId, String targetId) {
		...
		if (source == null || target == null || source == target || !source.isActive() || !target.isActive()
				|| source.isPuppet())
			return false;
		if (WarSystem.getInstance().areAtWar(sourceId, targetId))
			return false;
		if (!source.spendPoliticalPower(25.0))
			return false;
	...
	private static String slugify(String value) {
		String normalised = Normalizer.normalize(value, Normalizer.Form.NFKD).replaceAll("\\p{M}+", "")
