 ...
 *
 * One Minecraft day is 24,000 server ticks. The first full Minecraft day
 * advances exactly 100 Grand Strategy years. The rate then decelerates smoothly
 * as history progresses until the 2026 endpoint is exactly one Grand Strategy
 * day per Minecraft day.
 */
public final class HistoricalTimeline {
	 ...
	 * held constant for that day. This makes the first Minecraft day exactly 100 GS
	 * years rather than merely approximately 100 years.
	 *
	...
	public synchronized int advanceTimeline() {
		...
		double increment = finalTickOfMinecraftDay ? cachedGsDaysPerMinecraftDay - gsDaysAccumulatedThisMinecraftDay
		...
		if (Math.abs(partialDay - nearestWhole) < 1.0e-9)
			partialDay = nearestWhole;

		int daysToAdvance = (int) Math.floor(partialDay);
		if (daysToAdvance > 0)
			partialDay -= daysToAdvance;
	...
	/**
	 * Cheat/testing-only calendar jump. This changes Grand Strategy historical time
	 * directly without advancing Minecraft world time or replaying millions of
	 * ordinary server ticks.
	 *
	 ...
	 * history. Endpoints are exact: 36,500 GS days/MC day at the beginning and 1 GS
	 * day/MC day at 2026.
	 */
	private double calculateGsDaysPerMinecraftDay() {
		double progress = timelineProgress();
		if (progress <= 0.0)
			return START_GS_DAYS_PER_MINECRAFT_DAY;
		if (progress >= 1.0)
	...
	private double timelineProgress() {
		double elapsedDays = continuousDayIndex(currentYear, currentDay) - continuousDayIndex(START_YEAR, 0);
		double totalDays = continuousDayIndex(END_YEAR, 0) - continuousDayIndex(START_YEAR, 0);
	...
	private void synchroniseTimeline() {
		NetworkManager.getInstance().sendPacket(new SyncTimelinePacket((int) currentYear, currentEra.name()));
	}
	...
	public synchronized void restore(long year, int day, double partialDay, int minecraftDayTick, double cachedRate) {
		...
		this.cachedGsDaysPerMinecraftDay = Double.isFinite(cachedRate) && cachedRate >= 1.0 ? cachedRate
				: calculateGsDaysPerMinecraftDay();
		this.gsDaysAccumulatedThisMinecraftDay = this.cachedGsDaysPerMinecraftDay * this.minecraftDayTick
				/ MINECRAFT_TICKS_PER_DAY;
		updateEra();
	}

	public synchronized long getCurrentYear() {
		return currentYear;
	}
	...
	public synchronized double getPartialDay() {
		return partialDay;
	}

	public synchronized int getMinecraftDayTick() {
		return minecraftDayTick;
	}

	public synchronized double getGsDaysPerMinecraftDay() {
		return cachedGsDaysPerMinecraftDay;
	}

	public synchronized Era getCurrentEra() {
		return currentEra;
	}
	...
	public enum Era {
		PREHISTORIC, ANCIENT, MEDIEVAL, EARLY_MODERN, MODERN, CONTEMPORARY
	}
}
