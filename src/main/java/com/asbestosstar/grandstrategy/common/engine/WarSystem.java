 ...
 * accept the same peace proposal; total defeat (zero population or loss of
 * every controlled command post) instead causes immediate capitulation.
 * Historical/AI countries use the same proposal/acceptance path as player
 * countries when the war has not already ended through total defeat.
 */
public final class WarSystem {
	...
	public synchronized boolean declareWar(String attackerId, String defenderId) {
		if (attackerId == null || defenderId == null || attackerId.equals(defenderId))
			return false;
		...
		if (attacker == null || defender == null || !attacker.isActive() || !defender.isActive())
			return false;
		if (attacker.isPuppet())
			return false;
		if (activeWars.stream().anyMatch(war -> war.matchesEitherWay(attackerId, defenderId)))
			return false;
	...
	/**
	 * Called once per strategy step. No automatic war-progress or territorial
	...
	public synchronized void tick() {
		...
		for (War war : activeWars) {
			...
			processPeace(war);
			if (!war.over && war.pendingPeace == null)
				maybeOpenAiNegotiation(war);
			if (!war.over)
				processPeace(war);
		}
	...
	public synchronized boolean resolveZeroPopulation(Civilisation civilisation) {
		...
		for (War war : activeWars) {
			if (war.over || !war.involves(civilisation.getId()))
				continue;
			...
			if (capitulationWar == null || enemyAdvantage > bestEnemyAdvantage + 1.0e-9
					|| (Math.abs(enemyAdvantage - bestEnemyAdvantage) <= 1.0e-9 && enemyId.compareTo(victorId) < 0)) {
				capitulationWar = war;
				victorId = enemyId;
		...
		if (victorId != null) {
			...
			if (victor != null)
				victor.modifyRelation(civilisation.getId(), -30);
			...
			if (civilisation.collapse()) {
				System.out.println(
						"Civilisation " + civilisation.getName() + " reached zero population and was destroyed.");
			}
		}
	...
	/**
	 * Physical soldiers use this to distinguish wartime enemies from neutral
	 * villagers.
	 */
	 ...
	 * or by an unusual command-post transition. Only established countries
	 * currently involved in a war are eligible; a newly activated civilisation
	 * still waiting for its first discovered-land settlement is deliberately
	 * ignored.
	 */
	private void resolveCommandPostDefeats() {
		List<String> candidates = DataManager.getCivilisations().values().stream().filter(Objects::nonNull)
				.filter(Civilisation::isActive).filter(Civilisation::hasEstablishedHomeland)
	...
	private boolean resolveCommandPostDefeat(String defeatedId, String preferredVictorId) {
		if (defeatedId == null || defeatedId.isBlank())
			return false;
		Civilisation defeated = DataManager.getCivilisations().get(defeatedId);
		if (defeated == null || !defeated.isActive() || !defeated.hasEstablishedHomeland())
			return false;
		if (hasControlledCommandPost(defeatedId))
			return false;
		...
		if (preferredVictorId != null && !preferredVictorId.isBlank()) {
			...
			if (preferred != null && preferred.isActive() && findWar(defeatedId, preferredVictorId) != null) {
		...
		if (victorId == null) {
			...
			for (War candidate : activeWars) {
				if (candidate.over || !candidate.involves(defeatedId))
					continue;
				...
				if (victorId == null || enemyAdvantage > bestEnemyAdvantage + 1.0e-9
		...
		if (victor != null)
			victor.modifyRelation(defeatedId, -30);
		boolean collapsed = defeated.collapse();

		activeWars.removeIf(War::isOver);
		StrategyEngine.getInstance().requestSave();
		if (collapsed) {
			System.out.println("Civilisation " + defeated.getName() + " lost its final command post and capitulated to "
					+ (victor == null ? victorId : victor.getName()) + ".");
		}
		return collapsed;
	}
	...
	private boolean hasControlledCommandPost(String civilisationId) {
		...
		for (Providence providence : DataManager.getProvidences().values()) {
			if (providence == null || !providence.isEstablished() || providence.getCity() == null)
				continue;
			if (Objects.equals(civilisationId, providence.getCity().getControllerId()))
				return true;
		}
		return false;
	}
	 ...
	 * civilian workers because destroying an army matters more to a peace
	 * conference.
	 */
	 ...
	 * walking across a border is occupation rather than permanent annexation. If
	 * the opposing side later walks a soldier back onto the chunk, jurisdiction
	 * flips back.
	 */
	public synchronized void reportArmyPosition(String civilisationId, double blockX, double blockZ) {
		...
		if (currentController == null || currentController.isBlank()
				|| Objects.equals(currentController, civilisationId))
			return;
	...
	public synchronized void reportCommandPostCapture(String capturerId, String previousControllerId,
		...
		if (war == null || providenceId == null)
			return;
		Set<String> occupied = Objects.equals(capturerId, war.attackerId) ? war.attackerOccupiedProvidences
				: war.defenderOccupiedProvidences;
		Set<String> enemyOccupied = Objects.equals(capturerId, war.attackerId) ? war.defenderOccupiedProvidences
				: war.attackerOccupiedProvidences;
		...
		if (occupied.add(providenceId)) {
			if (Objects.equals(capturerId, war.attackerId))
				war.attackerOccupationScore += CITY_OCCUPATION_SCORE;
			else
				war.defenderOccupationScore += CITY_OCCUPATION_SCORE;
			war.targetProvidenceId = providenceId;
			war.recalculateScore();
			StrategyEngine.getInstance().requestSave();
			System.out.println(capturerId + " broke and captured command post " + providenceId + "; war score is now "
					+ (int) Math.round(war.warScore) + ".");
		}
	...
	public synchronized boolean shareCommonEnemy(String firstId, String secondId) {
		...
		for (War war : activeWars) {
			if (war.over)
				continue;
			if (war.involves(firstId))
				firstEnemies.add(war.opponentOf(firstId));
			if (war.involves(secondId))
				secondEnemies.add(war.opponentOf(secondId));
		}
	...
	/**
	 * Proposes a treaty. The proposer is considered to have accepted its own
	 * proposal.
	 */
	public synchronized boolean proposePeace(String proposerId, String opponentId, String encodedTerms) {
		...
		if (!PEACE_WHITE.equals(type)) {
			if (winnerId == null || loserId == null || !termUnlocked(type, advantage))
				return false;
		}

		String territoryId = null;
		if (PEACE_TERRITORY.equals(type) || PEACE_TERRITORY_AND_PUPPET.equals(type)) {
			territoryId = validCessionTerritory(loserId, requestedTerritory) ? requestedTerritory
					: recommendedTerritory(war);
			if (territoryId == null)
				return false;
		}
	...
	public synchronized boolean acceptPeace(String civilisationId, String opponentId) {
		...
		if (war == null || war.pendingPeace == null)
			return false;
		if (Objects.equals(civilisationId, war.attackerId))
			war.pendingPeace.acceptedByAttacker = true;
		else if (Objects.equals(civilisationId, war.defenderId))
			war.pendingPeace.acceptedByDefender = true;
		else
			return false;
		processPeace(war);
		return true;
	...
	public synchronized boolean rejectPeace(String civilisationId, String opponentId) {
		War war = findWar(civilisationId, opponentId);
		if (war == null || war.pendingPeace == null || !war.involves(civilisationId))
			return false;
		war.pendingPeace = null;
		return true;
	}
	...
	public synchronized void restore(List<WarState> states) {
		...
		for (WarState state : states) {
			if (state == null || state.attackerId == null || state.defenderId == null)
				continue;
			if (!DataManager.getCivilisations().containsKey(state.attackerId)
					|| !DataManager.getCivilisations().containsKey(state.defenderId))
				continue;
	...
	private void processPeace(War war) {
		if (war.pendingPeace == null || war.over)
			return;
	...
	private void autoAcceptIfAi(War war, String civilisationId) {
		...
		if (proposal == null)
			return;
		boolean alreadyAccepted = Objects.equals(civilisationId, war.attackerId) ? proposal.acceptedByAttacker
				: proposal.acceptedByDefender;
		if (alreadyAccepted)
			return;
		...
		if (PEACE_WHITE.equals(proposal.type)) {
			...
			accept = Objects.equals(civilisationId, proposal.winnerId)
					|| (Objects.equals(civilisationId, proposal.loserId) && -war.scoreFor(civilisationId) >= required);
		}
		if (accept) {
			if (Objects.equals(civilisationId, war.attackerId))
				proposal.acceptedByAttacker = true;
			else
				proposal.acceptedByDefender = true;
		}
	}

	private void maybeOpenAiNegotiation(War war) {
		if (war.ageTicks < AI_NEGOTIATION_MIN_TICKS || war.ageTicks % AI_NEGOTIATION_INTERVAL_TICKS != 0)
			return;
		...
		double advantage = Math.abs(war.warScore);
		String type;
		if (advantage >= COMBINATION_SCORE_REQUIRED)
			type = PEACE_TERRITORY_AND_PUPPET;
		else if (advantage >= PUPPET_SCORE_REQUIRED)
			type = PEACE_PUPPET;
		else if (advantage >= TERRITORY_SCORE_REQUIRED)
			type = PEACE_TERRITORY;
		else if (war.ageTicks >= 600 && advantage <= 12.0)
			type = PEACE_WHITE;
		else
			return;
		...
		String encoded = type;
		if ((PEACE_TERRITORY.equals(type) || PEACE_TERRITORY_AND_PUPPET.equals(type)) && territory != null)
			encoded += ":" + territory;
	...
	private void applyPeace(War war, PeaceProposal proposal) {
		...
		if (PEACE_TERRITORY.equals(proposal.type) || PEACE_TERRITORY_AND_PUPPET.equals(proposal.type)) {
		...
		if (PEACE_PUPPET.equals(proposal.type) || PEACE_TERRITORY_AND_PUPPET.equals(proposal.type)) {
			Civilisation loser = DataManager.getCivilisations().get(proposal.loserId);
			if (loser != null)
				loser.setOverlordCivilisationId(proposal.winnerId);
		...
		for (Providence providence : DataManager.getProvidences().values()) {
			...
			if (!Objects.equals(owner, war.attackerId) && !Objects.equals(owner, war.defenderId))
				continue;
			providence.setCountrysideControllerId(owner);
			if (providence.getCity() != null)
				providence.getCity().setControllerId(owner);
		}

		Civilisation attacker = DataManager.getCivilisations().get(war.attackerId);
		Civilisation defender = DataManager.getCivilisations().get(war.defenderId);
		if (attacker != null)
			attacker.modifyRelation(war.defenderId, -20);
		if (defender != null)
			defender.modifyRelation(war.attackerId, -20);
		war.over = true;
		war.pendingPeace = null;
		System.out.println(
				"Peace treaty accepted by both " + war.attackerId + " and " + war.defenderId + ": " + summary + ".");
	}
	...
	private static void transferProvidence(Providence providence, String newOwnerId) {
		...
		if (previousOwnerId != null && newOwnerId != null && !Objects.equals(previousOwnerId, newOwnerId)) {
			...
			if (previous != null && next != null && previous.getPopulation() > 0) {
				...
				int populationToTransfer = remainingProvidences <= 1 ? previous.getPopulation()
				...
				if (populationToTransfer > 0) {
					...
					int physical = PhysicalVillagerSystem.getInstance().transferResidentsForProvidence(previousOwnerId,
							newOwnerId, cityId, populationToTransfer);
					PhysicalVillagerSystem.getInstance().requestImmediateReconcile();
					System.out.println("Providence " + providence.getName() + " transferred " + populationToTransfer
							+ " population from " + previousOwnerId + " to " + newOwnerId + " (" + physical
							+ " already materialised residents changed allegiance).");
				}
			}
		}
		...
		if (providence.getCity() != null)
			providence.getCity().setControllerId(newOwnerId);
	...
	private String recommendedTerritory(War war) {
		...
		if (loser == null || winner == null)
			return null;
		Set<String> winnerOccupations = Objects.equals(winner, war.attackerId) ? war.attackerOccupiedProvidences
				: war.defenderOccupiedProvidences;
		return DataManager.getProvidences().values().stream().filter(Providence::isEstablished)
				.filter(providence -> Objects.equals(loser, providence.getOwnerId()))
				.sorted(Comparator
						.comparing((Providence p) -> winnerOccupations.contains(p.getId())
								|| p.hasWartimeOccupationBy(winner))
						.reversed().thenComparing(Providence::getDevelopment, Comparator.reverseOrder())
						.thenComparing(Providence::getId))
				.map(Providence::getId).findFirst().orElse(null);
	}

	private static boolean validCessionTerritory(String loserId, String providenceId) {
		if (loserId == null || providenceId == null || providenceId.isBlank())
			return false;
		Providence providence = DataManager.getProvidences().get(providenceId);
		return providence != null && providence.isEstablished() && Objects.equals(loserId, providence.getOwnerId());
	}
	...
	private static String chooseObjective(String defenderId) {
		return DataManager.getProvidences().values().stream().filter(Providence::isEstablished)
				...
				.sorted(Comparator.comparing((Providence p) -> p.getCity() != null && p.getCity().isSupplyCapital())
						.reversed().thenComparing(Providence::getDevelopment, Comparator.reverseOrder())
						.thenComparing(Providence::getId))
				.map(Providence::getId).findFirst().orElse(null);
	}
	...
	private War findWar(String firstCivilisationId, String secondCivilisationId) {
		if (firstCivilisationId == null || secondCivilisationId == null
				|| firstCivilisationId.equals(secondCivilisationId))
			return null;
		for (War war : activeWars) {
			if (!war.over && war.matchesEitherWay(firstCivilisationId, secondCivilisationId))
				return war;
		}
		return null;
	}
	...
	private static boolean validPeaceType(String type) {
		return PEACE_WHITE.equals(type) || PEACE_TERRITORY.equals(type) || PEACE_PUPPET.equals(type)
				|| PEACE_TERRITORY_AND_PUPPET.equals(type);
	}
	...
	private static double requiredScore(String type) {
		if (PEACE_TERRITORY.equals(type))
			return TERRITORY_SCORE_REQUIRED;
		if (PEACE_PUPPET.equals(type))
			return PUPPET_SCORE_REQUIRED;
		if (PEACE_TERRITORY_AND_PUPPET.equals(type))
			return COMBINATION_SCORE_REQUIRED;
		return 0.0;
	}
	...
	private static final class War {
		...
		String opponentOf(String civilisationId) {
			if (attackerId.equals(civilisationId))
				return defenderId;
			if (defenderId.equals(civilisationId))
				return attackerId;
			return null;
		}
		...
		void recalculateScorePreservingLegacy(double legacyScore) {
			...
			if (Math.abs(derived) > 0.0001)
				warScore = clamp(derived, -100.0, 100.0);
			else
				warScore = clamp(legacyScore == 0.0 ? warScore : legacyScore, -100.0, 100.0);
		}
	...
	/**
	 * Gson-friendly persisted/network war state. Positive warScore favours
	 * attacker.
	 */
	public static final class WarState {
		...
		public WarState(String attackerId, String defenderId, double attackerProgress, String targetProvidenceId,
				boolean countrysideCaptured, boolean cityCaptured) {
		...
		public boolean involves(String civilisationId) {
			return civilisationId != null && (civilisationId.equals(attackerId) || civilisationId.equals(defenderId));
		}

		public String opponentOf(String civilisationId) {
			if (civilisationId == null)
				return null;
			if (civilisationId.equals(attackerId))
				return defenderId;
			if (civilisationId.equals(defenderId))
				return attackerId;
			return null;
		}
