...
public final class WarSystem {
    ...
    public synchronized boolean declareWar(String attackerId, String defenderId) {
        return declareWar(attackerId, defenderId, "GENERIC", null, false);
    }

    /**
     * Starts either a conventional war or a limited special military operation.
     * War-goal metadata is persisted on the conflict and later reduces the peace
     * conference score needed for matching terms.
     */
    public synchronized boolean declareWar(String attackerId, String defenderId,
                                           String warGoalType, String warGoalTerritoryId,
                                           boolean specialOperation) {
        ...
        if (attacker.isAlliedWith(defenderId) || attacker.isInFactionWith(defenderId)) return false;
        ...
        war.warGoalType = warGoalType == null || warGoalType.isBlank() ? "GENERIC" : warGoalType;
        war.warGoalTerritoryId = validCessionTerritory(defenderId, warGoalTerritoryId)
                ? warGoalTerritoryId : null;
        war.specialOperation = specialOperation;
        war.targetProvidenceId = war.warGoalTerritoryId != null
                ? war.warGoalTerritoryId : chooseObjective(defenderId);
        activeWars.add(war);
        attacker.modifyRelation(defenderId, specialOperation ? -40 : -60);
        defender.modifyRelation(attackerId, specialOperation ? -50 : -60);
        System.out.println((specialOperation ? "Special military operation" : "War")
                + " started between " + attackerId + " and " + defenderId
                + " goal=" + war.warGoalType
                + (war.warGoalTerritoryId == null ? "" : " territory=" + war.warGoalTerritoryId)
                + ". It continues until peace is accepted or one side suffers total defeat.");
        return true;
    }

    /** Marks a limited operation as a full war after allied/guarantor intervention. */
    public synchronized boolean markEscalated(String firstId, String secondId) {
        War war = findWar(firstId, secondId);
        if (war == null || !war.specialOperation || war.escalated) return false;
        war.escalated = true;
        return true;
    }
    ...
    public synchronized boolean proposePeace(String proposerId, String opponentId, String encodedTerms) {
        ...
        if (!PEACE_WHITE.equals(type)) {
            if (winnerId == null || loserId == null || !termUnlocked(war, type, advantage)) return false;
        }

        // A non-escalated SMO is a limited conflict: puppet/regime terms are not
        // available, and territory can only be ceded where local support is more
        // favourable to the initiator than to the current corporatist owner.
        ...
        if (PEACE_TERRITORY.equals(type) || PEACE_TERRITORY_AND_PUPPET.equals(type)) {
            ...
            if (war.specialOperation && !war.escalated) {
                Providence limitedTerritory = DataManager.getProvidences().get(territoryId);
                if (!DiplomacySystem.isSmoTerritoryEligible(war.attackerId, war.defenderId, limitedTerritory)) {
                    return false;
                }
            }
        }
    ...
    public synchronized void restore(List<WarState> states) {
        ...
        for (WarState state : states) {
            ...
            war.warGoalType = state.warGoalType == null ? "GENERIC" : state.warGoalType;
            war.warGoalTerritoryId = state.warGoalTerritoryId;
            war.specialOperation = state.specialOperation;
            war.escalated = state.escalated;
    ...
    private WarState snapshotOf(War war) {
        ...
        state.warGoalType = war.warGoalType;
        state.warGoalTerritoryId = war.warGoalTerritoryId;
        state.specialOperation = war.specialOperation;
        state.escalated = war.escalated;
    ...
    private void autoAcceptIfAi(War war, String civilisationId) {
        ...
        if (PEACE_WHITE.equals(proposal.type)) {
            ...
            double required = requiredScore(war, proposal.type);
    ...
    private void maybeOpenAiNegotiation(War war) {
        ...
        String type;
        if ((!war.specialOperation || war.escalated)
                && termUnlocked(war, PEACE_TERRITORY_AND_PUPPET, advantage)) type = PEACE_TERRITORY_AND_PUPPET;
        else if ((!war.specialOperation || war.escalated)
                && termUnlocked(war, PEACE_PUPPET, advantage)) type = PEACE_PUPPET;
        else if (termUnlocked(war, PEACE_TERRITORY, advantage)) type = PEACE_TERRITORY;
    ...
    private String recommendedTerritory(War war) {
        ...
        if (Objects.equals(winner, war.attackerId) && war.warGoalTerritoryId != null) {
            Providence goal = DataManager.getProvidences().get(war.warGoalTerritoryId);
            if (goal != null && Objects.equals(loser, goal.getOwnerId())
                    && (!war.specialOperation || war.escalated
                        || DiplomacySystem.isSmoTerritoryEligible(war.attackerId, war.defenderId, goal))) {
                return goal.getId();
            }
        }
        ...
        return DataManager.getProvidences().values().stream()
                ...
                .filter(providence -> !war.specialOperation || war.escalated
                        || DiplomacySystem.isSmoTerritoryEligible(war.attackerId, war.defenderId, providence))
                .sorted(Comparator
    ...
    private void maintainObjective(War war) {
        if (war.warGoalTerritoryId != null) {
            Providence goal = DataManager.getProvidences().get(war.warGoalTerritoryId);
            if (goal != null && goal.isEstablished()
                    && Objects.equals(goal.getOwnerId(), war.defenderId)) {
                war.targetProvidenceId = goal.getId();
                return;
            }
        }
    ...
    private static boolean termUnlocked(War war, String type, double advantage) {
        return advantage + 1.0e-9 >= requiredScore(war, type);
    }

    private static double requiredScore(War war, String type) {
        double base;
        if (PEACE_TERRITORY.equals(type)) base = TERRITORY_SCORE_REQUIRED;
        else if (PEACE_PUPPET.equals(type)) base = PUPPET_SCORE_REQUIRED;
        else if (PEACE_TERRITORY_AND_PUPPET.equals(type)) base = COMBINATION_SCORE_REQUIRED;
        else return 0.0;

        if (war != null) {
            if ("TERRITORY".equals(war.warGoalType)
                    && (PEACE_TERRITORY.equals(type) || PEACE_TERRITORY_AND_PUPPET.equals(type))) {
                base -= 12.0;
            }
            if ("PUPPET".equals(war.warGoalType)
                    && (PEACE_PUPPET.equals(type) || PEACE_TERRITORY_AND_PUPPET.equals(type))) {
                base -= 15.0;
            }
            if (war.specialOperation && !war.escalated && PEACE_TERRITORY.equals(type)) {
    ...
    private static final class War {
        ...
        String warGoalType = "GENERIC";
        String warGoalTerritoryId;
        boolean specialOperation;
        boolean escalated;
    ...
    public static final class WarState {
        ...
        public String warGoalType = "GENERIC";
        public String warGoalTerritoryId;
