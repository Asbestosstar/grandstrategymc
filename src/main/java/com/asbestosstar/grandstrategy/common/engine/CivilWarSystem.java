...
/** Creates a real breakaway country when religious/ideological fragmentation becomes extreme. */
public final class CivilWarSystem {
    ...
    public static void maybeTrigger(Civilisation parent, long currentYear) {
        if (parent == null || !parent.isActive() || parent.getCivilWarCooldownSteps() > 0) return;
        if (parent.getPopulation() < 12 || parent.getStability() > 0.14) return;
        ...
        boolean religionCrisis = parent.getReligiousExtremism() >= 70.0
                && (religionFragmentation >= 0.42
                    || SocietySystem.religionSimilarity(parent.getStateReligionId(), parent.getPopulationPluralityReligion()) < 0.25
                    || (leaderReligion != null && SocietySystem.religionSimilarity(leaderReligion, parent.getStateReligionId()) < 0.20));
        boolean ideologyCrisis = parent.getIdeologicalExtremism() >= 70.0
                && (ideologyFragmentation >= 0.42
                    || SocietySystem.ideologySimilarity(parent.getIdeology(), parent.getPopulationPluralityIdeology()) < 0.25
                    || (leaderIdeology != null && SocietySystem.ideologySimilarity(leaderIdeology, parent.getIdeology()) < 0.20));
        if (!religionCrisis && !ideologyCrisis) return;
        ...
        while (DataManager.getCivilisations().containsKey(rebelId)) rebelId = baseId + "_" + (++suffix);
        ...
        Leader rebelLeader = new Leader(rebelLeaderId, parent.getName() + " Opposition Council", false,
                ...
                Math.max(55.0, parent.getReligiousExtremism()),
                Math.max(55.0, parent.getIdeologicalExtremism()));
        DataManager.getLeaders().put(rebelLeaderId, rebelLeader);
        Civilisation rebel = new Civilisation(rebelId,
                parent.getName() + " Opposition", rebelLeaderId,
                rebelIdeology, rebelReligion, List.of(rebelCapital.getId()),
                currentYear, parent.getMapXPercent(), parent.getMapYPercent());
        ...
        System.out.println("[GrandStrategy][CivilWar] " + parent.getName() + " fractured: "
                + rebel.getName() + " controls " + transferred.size() + " cities."
                + " Trigger=" + (religionCrisis ? "religion" : "ideology") + ".");
    }
}
