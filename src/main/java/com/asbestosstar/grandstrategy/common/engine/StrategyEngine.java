 ...
 *
 * The engine is restartable, cannot queue an unbounded number of AI jobs, and
 * applies worker results on the server tick thread rather than mutating live
 * game state from background threads.
 */
public final class StrategyEngine {
	 ...
	 */
	public static final long HISTORICAL_CIVILISATION_DELAY_TICKS = 5L * HistoricalTimeline.MINECRAFT_TICKS_PER_DAY;
	...
	public synchronized void start(Path worldRoot) {
		...
		this.running = true;
		System.out.println(
				"Strategy Engine started for world " + requestedRoot + " with " + parallelism + " simulation workers.");
	}

	/**
	 * Called once from the server's world tick, never from the title-screen/client
	 * tick.
	 */
	...
	private SimulationSnapshot captureSnapshot() {
		List<CivilisationSnapshot> civilisations = DataManager.getCivilisations().values().stream()
				...
				.map(civilisation -> new CivilisationSnapshot(civilisation.getId(), civilisation.getStability()))
				.toList();

		List<ProvidenceSnapshot> providences = DataManager.getProvidences().values().stream()
				.map(providence -> new ProvidenceSnapshot(providence.getId(), providence.getOwnerId(),
						providence.getResistanceLevel()))
				.toList();
	...
	private SimulationResult computeSimulation(SimulationSnapshot snapshot) {
		Set<String> owners = snapshot.providences().stream().map(ProvidenceSnapshot::ownerId)
		...
		Set<String> revolts = snapshot.providences().parallelStream()
				.filter(providence -> providence.resistanceLevel() > 0.8).map(ProvidenceSnapshot::id)
	...
	private void performParallelStandard(int[] data) {
		...
		pool.submit(() -> IntStream.of(data).parallel().forEach(value -> {
		})).join();
	}
	...
	public synchronized void stop() {
		...
		// Apply an already completed result, but never hold world shutdown waiting on
		// AI work.
		applyCompletedSimulation();
		if (pendingSimulation != null && !pendingSimulation.isDone()) {
	...
	private record SimulationSnapshot(List<CivilisationSnapshot> civilisations, List<ProvidenceSnapshot> providences) {
	}

	private record SimulationResult(Map<String, Double> stabilityByCivilisation, Set<String> revoltingProvidences) {
	}
}
