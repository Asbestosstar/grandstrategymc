 ...
 * This layer handles government/research/population bookkeeping and consumes
 * the physical chest-backed food and supply stockpiles.
 */
public final class CivilisationSimulation {
	...
	/**
	 * Legacy convenience tick. Historical activation is intentionally NOT done here
	 * because StrategyEngine owns the persisted five-Minecraft-day gate.
	 */
	...
	/**
	 * Checked every server tick so high prehistoric calendar rates do not skip
	 * start dates.
	 */
	public static void activateDueCivilisations(long currentYear) {
		for (Civilisation civilisation : DataManager.getCivilisations().values()) {
			if (civilisation.startIfDue(currentYear)) {
				...
				System.out.println("Civilisation " + civilisation.getName() + " started/restored in " + currentYear
						+ " with " + civilisation.getPopulation() + " physical villagers queued"
	...
	/**
	 * Economy remains a once-per-second strategy step rather than running 20x
	 * faster.
	 */
	public static void tickActiveEconomies() {
		for (Civilisation civilisation : DataManager.getCivilisations().values()) {
			if (civilisation.isActive())
				tickCivilisation(civilisation);
		}
	}
	...
	private static void tickCivilisation(Civilisation civilisation) {
		...
		// and are used only to decide whether the country has enough reserve for
		// births.
		...
		double ppGain = (0.035 + civilisation.getJobCount(VillagerJob.ADMINISTRATOR) * 0.008) * administration
				* Math.max(0.25, civilisation.getStability());
		...
		if (population > 1 && foodEquivalent >= requiredReserve) {
			double spiritMultiplier = civilisation.hasNationalSpirit(NationalSpirit.AGRARIAN_TRADITION) ? 1.25 : 1.0;
			...
			double populationAcceleration = 1.0
					+ Math.min(2.0, Math.max(0.0, population - Civilisation.STARTING_POPULATION)
							/ BIRTH_POPULATION_ACCELERATION_REFERENCE);
			double foodSurplusMultiplier = Math.min(1.75,
					Math.max(1.0, foodEquivalent / Math.max(requiredReserve, 1.0)));
			civilisation.addPopulationGrowthProgress(population * BASE_BIRTH_PROGRESS_PER_PERSON_PER_STEP
