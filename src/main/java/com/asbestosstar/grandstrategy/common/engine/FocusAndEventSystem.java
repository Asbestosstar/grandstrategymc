 ...
 *
 * Focus progress deliberately uses bounded strategy steps rather than
 * historical calendar days. Otherwise a 100-years-per-Minecraft-day prehistoric
 * clock would complete an entire focus tree in seconds. Random events also
 * occur in strategy time, while their trigger years still use the GS historical
 * timeline.
 */
public final class FocusAndEventSystem {
	...
	public static boolean canStartFocus(Civilisation civilisation, FocusTree.FocusNode node) {
		if (civilisation == null || node == null || !civilisation.isActive())
			return false;
		if (civilisation.getActiveFocusId() != null || civilisation.hasCompletedFocus(node.getId()))
			return false;
	...
	public static boolean startFocus(Civilisation civilisation, String focusId) {
		...
		return canStartFocus(civilisation, node) && civilisation.beginFocus(node.getId(), node.getPoliticalPowerCost());
	}
	...
	private static void tickFocus(Civilisation civilisation) {
		...
		if (activeId == null) {
			if (!civilisation.isPlayerCreated()) {
				...
				if (choice != null)
					startFocus(civilisation, choice.getId());
			}
			return;
		}
		...
		if (civilisation.advanceActiveFocus(1.0) + 1.0e-9 < active.getDurationSteps())
			return;

		if (civilisation.completeFocus(active.getId())) {
			applyEffects(civilisation, active.getEffects());
			System.out
					.println("Grand Strategy focus completed: " + civilisation.getName() + " -> " + active.getTitle());
		}
	}
	...
	private static FocusTree.FocusNode weightedFocusChoice(Civilisation civilisation, FocusTree tree) {
		List<FocusTree.FocusNode> candidates = tree.getNodes().stream()
				.filter(node -> canStartFocus(civilisation, node)).toList();
	...
	private static void tickEvents(Civilisation civilisation, long currentYear) {
		...
		if (civilisation.hasPendingEvent() || civilisation.getEventCooldownSteps() > 0)
			return;
		if (DataManager.getEvents().isEmpty())
			return;
		if (ThreadLocalRandom.current().nextDouble() >= EVENT_CHANCE_PER_STRATEGY_STEP)
			return;
		...
		List<GrandStrategyEvent> candidates = DataManager.getEvents().values().stream()
				...
				.sorted(Comparator.comparing(GrandStrategyEvent::getId)).toList();
		if (candidates.isEmpty())
			return;

		GrandStrategyEvent event = weighted(candidates, GrandStrategyEvent::getWeight);
		if (event == null || !civilisation.queueEvent(event.getId()))
			return;
		...
		if (!civilisation.isPlayerCreated()) {
			...
			if (option != null)
				resolveEvent(civilisation, event.getId(), option.getId());
		}
	}
	...
	private static boolean recentlyResolved(Civilisation civilisation, String eventId) {
		...
		for (int i = from; i < history.size(); i++) {
			if (history.get(i).startsWith(eventId + ":"))
				return true;
		}
		return false;
	}
	...
	public static boolean resolveEvent(Civilisation civilisation, String eventId, String optionId) {
		if (civilisation == null || eventId == null || optionId == null
				|| !eventId.equals(civilisation.getPendingEventId()))
			return false;
	...
	public static void applyEffects(Civilisation civilisation, List<StrategyEffect> effects) {
		if (civilisation == null || effects == null)
			return;
		for (StrategyEffect effect : effects)
			applyEffect(civilisation, effect);
	}
	...
	private static void applyEffect(Civilisation civilisation, StrategyEffect effect) {
		...
		case RESOURCE -> {
			...
			if (type != null)
				civilisation.addResource(type, effect.getAmount());
		}
		...
		case POPULATION -> {
			...
			if (amount > 0)
				civilisation.addPopulation(amount);
			else if (amount < 0)
				civilisation.removePopulation(-amount);
		}
		...
		case CONSCRIPTION -> {
			...
			if (level != null)
				civilisation.setConscriptionLevel(level);
		}
	...
	private static Civilisation nearestActiveCivilisation(Civilisation source) {
		...
		for (Civilisation candidate : DataManager.getCivilisations().values()) {
			if (candidate != source && candidate.isActive())
				candidates.add(candidate);
		}
		if (candidates.isEmpty())
			return null;
		if (!source.hasWorldMapPosition())
			return candidates.get(0);

		Civilisation best = null;
		long bestDistance = Long.MAX_VALUE;
		for (Civilisation candidate : candidates) {
			if (!candidate.hasWorldMapPosition())
				continue;
	...
	private static <T> T weighted(List<T> values, java.util.function.ToDoubleFunction<T> weightFunction) {
		...
		double total = 0.0;
		for (T value : values)
			total += Math.max(0.0, weightFunction.applyAsDouble(value));
		if (total <= 0.0)
