...
public final class SocietySystem {
	...
	public static void tick(long currentYear) {
		...
		if (stepCounter % 60L == 0L)
			applyReligiousAndIdeologicalDiplomacy();
	}
	...
	public static boolean religionAvailable(String religionId, long currentYear) {
		...
		if (religion == null || currentYear < religion.getCreationYear())
			return false;
	...
	private static void announceNewReligions(long currentYear) {
		List<Religion> religions = DataManager.getReligions().values().stream()
				...
				.sorted(Comparator.comparingLong(Religion::getCreationYear)).toList();
		for (Religion religion : religions) {
			if (!ANNOUNCED_RELIGIONS.add(religion.getId()))
				continue;
			...
			if (religion.getCreationYear() != Long.MIN_VALUE) {
				System.out.println("[GrandStrategy][Religion] " + religion.getName() + " emerged"
						+ (origin == null ? "" : " in " + origin.getName()) + " in " + formatYear(currentYear) + ".");
			}
		}
	}

	/**
	 * Historical religion creation is a real city event when one of the religion's
	 * configured origin cities exists. The city keeps its own name regardless of
	 * the country currently controlling it; the controller is merely the population
	 * that receives the first substantial conversion pressure. If none of the
	 * configured cities exists, the religion still becomes globally available at
	 * its historical date so alternate-history worlds cannot permanently deadlock
	 * the religion tree.
	 */
	private static void triggerReligionEmergenceEvent(Religion religion, City origin) {
		if (religion == null || origin == null || !religion.isPopulationAllowed())
			return;
		...
		System.out.println("[GrandStrategy][ReligionEvent] " + origin.getName() + " became the origin centre of "
				+ religion.getName() + "; first converts appeared in " + controller.getName() + ".");
	}

	private static City findOriginCity(Religion religion) {
		if (religion == null || religion.getOriginCityNames().isEmpty())
			return null;
		for (String wanted : religion.getOriginCityNames()) {
			for (Providence providence : DataManager.getProvidences().values()) {
				...
				if (city != null && wanted.equalsIgnoreCase(city.getName()))
					return city;
			}
		}
		return null;
	...
	private static void applyLeaderAndStateSnaps(Civilisation civilisation, long currentYear) {
		...
		if (leader != null) {
			...
			if (snap != null && !snap.equals(leader.getReligionId()) && religion != null && religion.isLeaderAllowed()
					&& religionAvailable(snap, currentYear)) {
				leader.setReligionId(snap);
				System.out.println(
						"[GrandStrategy][Religion] " + leader.getName() + " adopted " + religion.getName() + ".");
			}
		...
		if (stateSnap != null && !stateSnap.equals(civilisation.getStateReligionId()) && state != null
				&& religionAvailable(stateSnap, currentYear)) {
		...
		if (leader != null && stepCounter % 300L == 0L) {
			...
			if (stateReligion != null && !"secular".equalsIgnoreCase(stateReligion) && leaderReligion != null
					&& !stateReligion.equals(leaderReligion)) {
				...
				if (statePressure >= leaderPressure + 12.0) {
					...
					if (stateDefinition != null && stateDefinition.isLeaderAllowed())
						leader.setReligionId(stateReligion);
	...
	private static void applyCohesion(Civilisation civilisation) {
		...
		double ideologyLeaderState = leaderIdeology == null ? ideologyStatePeople
				: ideologySimilarity(leaderIdeology, stateIdeology);
		double ideologyLeaderPeople = leaderIdeology == null ? ideologyStatePeople
				: ideologySimilarity(leaderIdeology, pluralityIdeology);
		double ideologyAlignment = ideologyStatePeople * 0.55 + ideologyLeaderState * 0.20
				+ ideologyLeaderPeople * 0.25;
	...
	/**
	 * 0=identical sect, partial=same family, 0=different; secular/nonreligious have
	...
	public static double religionSimilarity(String firstId, String secondId) {
		...
		if (first == null || second == null)
			return 0.0;
		if (Objects.equals(first.getFamilyId(), second.getFamilyId()))
			return 0.68;
	...
	public static double ideologySimilarity(String firstId, String secondId) {
		...
		if (Objects.equals(first.getFamilyId(), second.getFamilyId()))
			return 0.70;
		if (first.isNonAligned() || second.isNonAligned())
			return 0.25;
		return 0.0;
	}
	...
	public static double fragmentation(Map<String, Double> shares) {
		...
		for (double share : shares.values())
			sumSquares += Math.max(0.0, share) * Math.max(0.0, share);
	...
	private static void applyReligiousAndIdeologicalDiplomacy() {
		...
		for (int i = 0; i < active.size(); i++) {
			...
			for (int j = i + 1; j < active.size(); j++) {
				...
				if (delta < 0 && !a.isPuppet() && !b.isPuppet() && a.getRelation(b.getId()) <= -90
	...
	public static double militaryMoraleMultiplier(Civilisation civilisation) {
		...
		double extremism = civilisation.getReligiousExtremism() / 100.0;
		if (state != null && state.isNonviolent())
	...
	private static Civilisation nearestOtherCountry(Civilisation civilisation) {
		if (civilisation == null || !civilisation.hasWorldMapPosition())
			return null;
		Civilisation best = null;
		double bestDistance = Double.POSITIVE_INFINITY;
		for (Civilisation other : DataManager.getCivilisations().values()) {
			if (other == null || other == civilisation || !other.isActive() || !other.hasWorldMapPosition())
				continue;
