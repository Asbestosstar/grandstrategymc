...
public class PoliticsScreen extends StrategyScreen {
    ...
    protected void init() {
        beginIconLayout();
        int x = 6;
        addIconButton(UiIcon.MAP, "Map", x, 6, button ->
                this.minecraft.setScreen(new MapScreen(getParentScreen())));
        x += 24;
        addIconButton(UiIcon.DIPLOMACY, "Diplomacy", x, 6, button ->
                this.minecraft.setScreen(new DiplomacyScreen(this)));
        x += 24;
        addIconButton(UiIcon.FOCUS, "Focus tree", x, 6, button ->
        ...
        addIconButton(UiIcon.EVENTS,
                own != null && own.hasPendingEvent() ? "Events - decision waiting" : "Events & decisions",
                x, 6, button -> this.minecraft.setScreen(new EventsScreen(this)));
        x += 24;
        addIconButton(UiIcon.SPIRITS, "National spirits", x, 6, button ->
        ...
        if (own != null) {
            this.addRenderableWidget(Button.builder(Component.literal("Research"), button ->
                    ...
                    .bounds(x, 6, 82, 20).build());
            x += 86;
            this.addRenderableWidget(Button.builder(Component.literal("Religion & Ideology"), button ->
                            this.minecraft.setScreen(new SocietyScreen(this)))
    ...
    protected void renderCustom(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        if (own != null) {
            ...
            graphics.text(this.font, "Your government: " + own.getGovernment()
                    + " | Focus: " + focus + " | Event: " + event, 42, y + 1,
                    own.hasPendingEvent() ? WARNING_TEXT : TEXT, true);
            y += 20;
        }

        for (Civilisation civ : StrategyClientContext.civilisations()) {
            String text = "Civ: " + civ.getName()
                    + " | Government: " + civ.getIdeology()
                    + " | Focuses completed: " + civ.getCompletedFocusIds().size()
                    + " | Status: " + (civ.isActive() ? "Active" : "Potential");
