...
public final class StrategyClientContext {
	...
	public static Civilisation currentPlayerCountry() {
		...
		for (Civilisation civilisation : civilisations()) {
			if (civilisation == null || !civilisation.isPlayerCreated() || !civilisation.isActive()
					|| civilisation.isCollapsed() || civilisation.getPopulation() <= 0)
				continue;
			if (uuid != null && uuid.equalsIgnoreCase(civilisation.getFounderUuid()))
				return civilisation;
			if ((civilisation.getFounderUuid() == null || civilisation.getFounderUuid().isBlank())
					&& civilisation.getFounderIgn() != null && civilisation.getFounderIgn().equalsIgnoreCase(ign)) {
				return civilisation;
			}
		}
		...
		Minecraft minecraft = Minecraft.getInstance();
		if (!StrategyClientState.getInstance().isSynchronised() && minecraft.getSingleplayerServer() != null) {
	...
	public static long currentYear() {
		...
		if (state.isSynchronised())
			return state.getCurrentYear();
		Minecraft minecraft = Minecraft.getInstance();
	...
	public static Collection<Civilisation> civilisations() {
		StrategyClientState state = StrategyClientState.getInstance();
		if (state.isSynchronised())
			return state.getCivilisations().values();

		Minecraft minecraft = Minecraft.getInstance();
		if (minecraft.getSingleplayerServer() != null) {
			return DataManager.getCivilisations().values().stream().filter(Civilisation::isActive).toList();
		}
		return List.of();
	}
	...
	public static Collection<Providence> providences() {
		StrategyClientState state = StrategyClientState.getInstance();
		if (state.isSynchronised())
			return state.getProvidences().values();
		Minecraft minecraft = Minecraft.getInstance();
		if (minecraft.getSingleplayerServer() != null) {
			return DataManager.getProvidences().values().stream().filter(Providence::isEstablished).toList();
		}
		return List.of();
	}
	...
	public static Civilisation getCivilisation(String id) {
		...
		StrategyClientState state = StrategyClientState.getInstance();
		if (state.isSynchronised())
			return state.getCivilisation(id);
		Minecraft minecraft = Minecraft.getInstance();
	...
	public static FocusTree focusTreeFor(Civilisation civilisation) {
		...
		StrategyClientState state = StrategyClientState.getInstance();
		if (state.isSynchronised())
			return state.getFocusTree(civilisation.getId());
		Minecraft minecraft = Minecraft.getInstance();
		if (minecraft.getSingleplayerServer() == null)
			return null;
	...
	public static Collection<Technology> technologies() {
		StrategyClientState state = StrategyClientState.getInstance();
		if (state.isSynchronised())
			return state.getTechnologies().values();
		Minecraft minecraft = Minecraft.getInstance();
	...
	public static Collection<FactoryType> factoryTypes() {
		StrategyClientState state = StrategyClientState.getInstance();
		if (state.isSynchronised())
			return state.getFactoryTypes().values();
		Minecraft minecraft = Minecraft.getInstance();
	...
	public static Collection<FactoryRecipe> factoryRecipes() {
		StrategyClientState state = StrategyClientState.getInstance();
		if (state.isSynchronised())
			return state.getFactoryRecipes().values();
		Minecraft minecraft = Minecraft.getInstance();
	...
	public static Collection<Religion> religions() {
		StrategyClientState state = StrategyClientState.getInstance();
		if (state.isSynchronised())
			return state.getReligions().values();
		Minecraft minecraft = Minecraft.getInstance();
	...
	public static Collection<Ideology> ideologies() {
		StrategyClientState state = StrategyClientState.getInstance();
		if (state.isSynchronised())
			return state.getIdeologies().values();
		Minecraft minecraft = Minecraft.getInstance();
	...
	public static List<PhysicalVillagerSystem.VillagerMapMarker> villagers() {
		StrategyClientState state = StrategyClientState.getInstance();
		if (state.isSynchronised())
			return state.getVillagers();
		Minecraft minecraft = Minecraft.getInstance();
		return minecraft.getSingleplayerServer() == null ? List.of()
	...
	public static WarSystem.WarState warBetween(String firstId, String secondId) {
		for (WarSystem.WarState war : wars()) {
			if (war == null)
				continue;
			if ((firstId != null && firstId.equals(war.attackerId) && secondId != null
					&& secondId.equals(war.defenderId))
					|| (firstId != null && firstId.equals(war.defenderId) && secondId != null
							&& secondId.equals(war.attackerId))) {
				return war;
			}
		}
		return null;
	...
	public static WorldMapTracker.Snapshot mapSnapshot() {
		StrategyClientState state = StrategyClientState.getInstance();
		if (state.isSynchronised())
			return state.getMapSnapshot();
		Minecraft minecraft = Minecraft.getInstance();
		return minecraft.getSingleplayerServer() == null ? WorldMapTracker.Snapshot.empty()
	...
	public static double gsDaysPerMinecraftDay() {
		StrategyClientState state = StrategyClientState.getInstance();
		if (state.isSynchronised())
			return state.getGsDaysPerMinecraftDay();
		return StrategyEngine.getInstance().getTimeline().getGsDaysPerMinecraftDay();
	}

	public static boolean requestSync() {
		return NetworkManager.getInstance().requestSync();
	}

	public static boolean requestCreateCountry() {
		return NetworkManager.getInstance().requestCreateCountry();
	}

	public static boolean requestImproveRelations(String target) {
		return NetworkManager.getInstance().requestImproveRelations(target);
	}

	public static boolean requestDeclareWar(String target) {
		return NetworkManager.getInstance().requestDeclareWar(target);
	}

	public static boolean requestPeaceProposal(String target, String terms) {
		return NetworkManager.getInstance().requestPeaceProposal(target, terms);
	}

	public static boolean requestPeaceAccept(String target) {
		return NetworkManager.getInstance().requestPeaceAccept(target);
	}

	public static boolean requestPeaceReject(String target) {
		return NetworkManager.getInstance().requestPeaceReject(target);
	}

	public static boolean requestSoldierAutomatic() {
		return NetworkManager.getInstance().requestSoldierAutomatic();
	}

	public static boolean requestSoldierMove(int blockX, int blockZ) {
		return NetworkManager.getInstance().requestSoldierMove(blockX, blockZ);
	}

	public static boolean requestWorkZone(String type, int blockX, int blockZ) {
		return NetworkManager.getInstance().requestWorkZone(type, blockX, blockZ);
	}

	public static boolean requestStartTechnology(String technologyId) {
		return NetworkManager.getInstance().requestStartTechnology(technologyId);
	}

	public static boolean requestQueueProduction(String recipeId, int amount) {
		return NetworkManager.getInstance().requestQueueProduction(recipeId, amount);
	}

	public static boolean requestCancelProduction(long serial) {
		return NetworkManager.getInstance().requestCancelProduction(serial);
	}

	public static boolean requestFactoryConversion(String zoneId, String factoryTypeId) {
		return NetworkManager.getInstance().requestFactoryConversion(zoneId, factoryTypeId);
	}

	public static boolean requestCycleConscription() {
		return NetworkManager.getInstance().requestCycleConscription();
	}

	public static boolean requestCycleGovernment() {
		return NetworkManager.getInstance().requestCycleGovernment();
	}

	public static boolean requestAutoAssign() {
		return NetworkManager.getInstance().requestAutoAssign();
	}

	public static boolean requestReassignTo(VillagerJob job) {
		return NetworkManager.getInstance().requestReassignTo(job);
	}

	public static boolean requestReassignFrom(VillagerJob job) {
		return NetworkManager.getInstance().requestReassignFrom(job);
	}

	public static boolean requestToggleSpirit(String spiritId) {
		return NetworkManager.getInstance().requestToggleSpirit(spiritId);
	}

	public static boolean requestStartFocus(String focusId) {
		return NetworkManager.getInstance().requestStartFocus(focusId);
	}

	public static boolean requestResolveEvent(String eventId, String optionId) {
		return NetworkManager.getInstance().requestResolveEvent(eventId, optionId);
	}
	...
	public static Integer currentPlayerBlockX() {
		Minecraft minecraft = Minecraft.getInstance();
		if (minecraft.player == null || !isPlayerInOverworld())
			return null;
