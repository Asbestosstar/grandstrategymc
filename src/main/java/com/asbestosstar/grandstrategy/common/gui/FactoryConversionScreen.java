...
public final class FactoryConversionScreen extends StrategyScreen {
    ...
    public FactoryConversionScreen(Screen parent) { super("Convert Factories", parent); }
    @Override protected void init() {
        ...
        List<FactoryType> unlocked = StrategyClientContext.factoryTypes().stream().filter(t -> t.isStarter() || t.getRequiredTechnologyIds().stream().allMatch(civ::hasTechnology)).sorted(Comparator.comparing(FactoryType::getName)).toList();
        List<PhysicalVillagerSystem.WorkZoneMapMarker> zones = StrategyClientContext.workZones().stream().filter(z -> z != null && civ.getId().equals(z.civilisationId()) && "FACTORY".equals(z.type())).toList();
        int y=58;
        for (PhysicalVillagerSystem.WorkZoneMapMarker zone: zones) {
            ...
            this.addRenderableWidget(Button.builder(Component.literal(readable(current)+" -> "+(next==null?"none":next.getName())), b -> {
                if (next != null && StrategyClientContext.requestFactoryConversion(zone.id(), next.getId())) { status="Conversion requested."; scheduleRefresh(); }
            }).bounds(22,y,260,20).build());
            y+=26;
        }
    }
    private static FactoryType nextType(List<FactoryType> list,String current) { if(list.isEmpty())return null; for(int i=0;i<list.size();i++) if(list.get(i).getId().equals(current)) return list.get((i+1)%list.size()); return list.get(0); }
    private static String readable(String s){ if(s==null)return "Wooden factory"; s=s.replace('_',' '); return Character.toUpperCase(s.charAt(0))+s.substring(1); }
