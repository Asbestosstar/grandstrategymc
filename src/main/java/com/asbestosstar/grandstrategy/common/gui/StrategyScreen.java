...
public abstract class StrategyScreen extends Screen {
	...
	protected enum UiIcon {
		BACK, MAP, ECONOMY, DIPLOMACY, SPIRITS, FOCUS, EVENTS, FARM_ZONE, FACTORY_ZONE, ARMY_AUTO, MOVE_ARMY, ZOOM_OUT,
		ZOOM_IN, PLAYER, FIT
	}
	 ...
	 */
	protected Button addIconButton(UiIcon icon, String tooltip, int x, int y, Consumer<Button> onPress) {
		return addIconButton(icon, tooltip, x, y, 20, 20, 0xFFD7DEE7, 0xFFFFFFFF, 0, onPress);
	}

	protected Button addIconButton(UiIcon icon, String tooltip, int x, int y, int width, int height,
			Consumer<Button> onPress) {
		return addIconButton(icon, tooltip, x, y, width, height, 0xFFD7DEE7, 0xFFFFFFFF, 0, onPress);
	}

	protected Button addIconButton(UiIcon icon, String tooltip, int x, int y, int width, int height, int colour,
			int hoverColour, int accentFill, Consumer<Button> onPress) {
	...
	public boolean isPauseScreen() {
		// World simulation and queued strategy actions must continue while this GUI is
		// open.
		return false;
	}
	...
	private void renderIconControls(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
		...
		for (IconControl control : iconControls) {
			...
			if (control.accentFill() != 0) {
				...
				graphics.fill(control.x() + 1, control.y() + 1, control.x() + control.width() - 1,
						control.y() + control.height() - 1, fill);
				graphics.outline(control.x() + 1, control.y() + 1, control.width() - 2, control.height() - 2,
	...
	private void drawTooltip(GuiGraphicsExtractor graphics, int mouseX, int mouseY, String text) {
		...
		int boxY = mouseY + 12;
		if (boxY + boxHeight > this.height - 4)
			boxY = Math.max(4, mouseY - boxHeight - 6);
	...
	private static void drawIcon(GuiGraphicsExtractor graphics, UiIcon icon, int x, int y, int width, int height,
			int colour) {
		int cx = x + width / 2;
		int cy = y + height / 2;
		switch (icon) {
	...
	private record IconControl(UiIcon icon, String tooltip, int x, int y, int width, int height, int colour,
			int hoverColour, int accentFill) {
