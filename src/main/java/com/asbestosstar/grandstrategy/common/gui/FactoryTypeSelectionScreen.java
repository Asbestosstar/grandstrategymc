...
public final class FactoryTypeSelectionScreen extends StrategyScreen {
	public FactoryTypeSelectionScreen(Screen parent) {
		super("Add Factory District", parent);
	}
	...
	protected void init() {
		...
		for (FactoryType type : types) {
			...
			Button button = Button
					.builder(Component.literal((unlocked ? "Build " : "Locked: ") + type.getName()), b -> {
						if (unlocked)
							this.minecraft.setScreen(new MapScreen(this, "FACTORY:" + type.getId()));
	...
	protected void renderCustom(GuiGraphicsExtractor g, int mx, int my, float pt) {
		...
		if (civ == null)
			return;
		int y = 62;
		for (FactoryType type : StrategyClientContext.factoryTypes().stream()
				.sorted(Comparator.comparing(FactoryType::getName)).toList()) {
			if (y > this.height - 24)
				break;
			String req = type.getRequiredTechnologyIds().isEmpty() ? "starter"
					: "requires " + String.join(", ", type.getRequiredTechnologyIds());
			g.text(this.font, type.getDescription() + " [" + req + "]", 212, y,
					unlocked(civ, type) ? MUTED_TEXT : BAD_TEXT, true);
			y += 28;
		}
	}

	private static boolean unlocked(Civilisation civ, FactoryType type) {
		if (type == null || civ == null)
			return false;
		if (type.isStarter())
			return true;
	...
	protected StrategyScreen recreate() {
		return new FactoryTypeSelectionScreen(getParentScreen());
	}
}
