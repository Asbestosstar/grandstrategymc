...
public final class CountryScreen extends StrategyScreen {
    ...
    protected void renderCustom(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        graphics.text(this.font,
                civilisation.getName() + " [" + civilisation.getId() + "]",
                18, y, TEXT, true);
        y += 18;
        graphics.text(this.font,
                "Status: " + (civilisation.isActive() ? "Active" : "Potential")
                        + " | Start: " + formatYear(civilisation.getStartYear())
                        + " | Stability: " + (int) (civilisation.getStability() * 100.0) + "%",
                18, y, civilisation.isActive() ? GOOD_TEXT : MUTED_TEXT, true);
        y += 16;
        graphics.text(this.font,
                "Government: " + civilisation.getIdeology()
                        + " | Religion: " + civilisation.getReligion()
        ...
        if (!civilisation.isActive()) {
            graphics.text(this.font,
                    "This civilisation is visible on the strategic map but has not started yet.",
                    18, y, WARNING_TEXT, true);
            return;
        }

        graphics.text(this.font,
                "Villagers: " + civilisation.getPopulation()
        ...
        graphics.text(this.font,
                "Resources: physical supply-depot chest inventory (numerical duplicate hidden)",
                18, y, TEXT, true);
        y += 20;

        graphics.text(this.font,
                "Political Power: " + (int) civilisation.getPoliticalPower()
                        + " | Research: " + (int) civilisation.getResearchPoints()
                        + " | Conscription: " + civilisation.getConscriptionLevel().getDisplayName(),
                18, y, MUTED_TEXT, true);
        y += 16;
        ...
        var providences = StrategyClientContext.providences().stream()
                ...
                .sorted(java.util.Comparator.comparing(Providence::getName))
                .toList();
        graphics.text(this.font, "Providences: " + providences.size()
                        + " | Border ARGB: #" + String.format(java.util.Locale.ROOT, "%08X", civilisation.getBorderColourArgb()),
                18, y, TEXT, true);
        y += 14;
        for (Providence providence : providences) {
            ...
            String capital = providence.getCity() != null && providence.getCity().isSupplyCapital()
                    ? " [SUPPLY CAPITAL]" : "";
            graphics.text(this.font,
                    "- " + providence.getName() + " | " + city + capital
                            + " | Supply " + (int) (providence.getSupplyLevel() * 100.0) + "%",
