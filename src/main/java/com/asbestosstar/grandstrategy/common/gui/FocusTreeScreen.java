...
public final class FocusTreeScreen extends StrategyScreen {
    ...
    protected void init() {
        ...
        if (civilisation == null || tree == null) return;
        ...
        for (FocusTree.FocusNode node : tree.getNodes()) {
            ...
            if (canStartLocally(civilisation, node)) {
                this.addRenderableWidget(Button.builder(Component.literal(label), button -> {
                            ...
                            statusMessage = queued ? "Focus request sent to server." : "Not connected to a Grand Strategy server.";
    ...
    private static boolean canStartLocally(Civilisation civilisation, FocusTree.FocusNode node) {
        if (civilisation == null || node == null || civilisation.getActiveFocusId() != null
                || civilisation.hasCompletedFocus(node.getId())) return false;
    ...
    protected void renderCustom(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        if (civilisation == null) {
            graphics.text(this.font, "Create a country before selecting national focuses.",
                    18, 58, WARNING_TEXT, true);
            return;
        }
        ...
        graphics.text(this.font,
                civilisation.getName() + " | Political Power " + (int) civilisation.getPoliticalPower()
                        + " | Completed " + civilisation.getCompletedFocusIds().size(),
    ...
    private static String requirementText(FocusTree.FocusNode node) {
        ...
        if (!prerequisites.isEmpty()) text.append(" | requires ").append(String.join(", ", prerequisites));
        if (!exclusive.isEmpty()) text.append(" | excludes ").append(String.join(", ", exclusive));
    ...
    public void tick() {
        super.tick();
        if (++syncTicks % 20 == 0) StrategyClientContext.requestSync();
        Civilisation civilisation = StrategyClientContext.currentPlayerCountry();
    ...
    private static String stateSignature(Civilisation civilisation) {
        if (civilisation == null) return "none";
        return String.valueOf(civilisation.getActiveFocusId()) + "|"
                + civilisation.getCompletedFocusIds().size() + "|"
