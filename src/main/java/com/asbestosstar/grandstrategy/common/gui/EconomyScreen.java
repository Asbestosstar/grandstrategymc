...
public final class EconomyScreen extends StrategyScreen {
    ...
    protected void init() {
        ...
        this.addRenderableWidget(Button.builder(
                        Component.literal("Conscription: " + civilisation.getConscriptionLevel().getDisplayName()),
                        button -> {
        ...
        for (VillagerJob job : VillagerJob.values()) {
            if (job != VillagerJob.SOLDIER) {
                ...
                addIconButton(UiIcon.ZOOM_OUT, "Remove one from " + selectedJob.getDisplayName(),
                        this.width - 52, rowY - 4, 20, 18, button -> {
                            boolean queued = StrategyClientContext.requestReassignFrom(selectedJob);
                            if (!queued) statusMessage = "Not connected to a Grand Strategy server.";
                            else scheduleRefresh();
                        });
                addIconButton(UiIcon.ZOOM_IN, "Assign one to " + selectedJob.getDisplayName(),
                        this.width - 28, rowY - 4, 20, 18, button -> {
        ...
        this.addRenderableWidget(Button.builder(Component.literal("Next >"), button -> {
                    ...
                    if (workerPage + 1 < pages) workerPage++;
    ...
    protected void renderCustom(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        if (civilisation == null) {
            graphics.text(this.font, "Create a country from the map before assigning villagers.",
                    18, 64, WARNING_TEXT, true);
            return;
        }

        graphics.text(this.font,
                civilisation.getName() + " | Population: " + civilisation.getPopulation()
                        + " | Assigned: " + civilisation.totalAssignedVillagers()
                        + " | Soldiers: " + civilisation.getJobCount(VillagerJob.SOLDIER),
                18, 56, TEXT, true);
        ...
        int infoY = rowY + 2;
        graphics.text(this.font,
                "Factories: " + civilisation.getFactories()
                        + "  progress " + percent(civilisation.getFactoryConstructionProgress())
                        + "   |   Roads: " + civilisation.getRoadSegments()
                        + "  progress " + percent(civilisation.getRoadConstructionProgress()),
                18, infoY, MUTED_TEXT, true);
        graphics.text(this.font,
                "Political Power: " + format(civilisation.getPoliticalPower())
                        + "   |   Research: " + format(civilisation.getResearchPoints())
                        + "   |   Birth progress: " + percent(civilisation.getPopulationGrowthAccumulator() * 100.0),
    ...
    private void renderWorkerTable(GuiGraphicsExtractor graphics, int tableY) {
        ...
        if (workerPage >= pages) workerPage = pages - 1;
        ...
        for (int index = start; index < end; index++) {
            ...
            graphics.text(this.font, fitText(status, statusWidth), statusX, y + 3,
                    statusColour(status), true);

            if (positionX < tableRight - 20) {
                String position = worker.blockX() + "," + worker.blockY() + "," + worker.blockZ();
                graphics.text(this.font, fitText(position, tableRight - positionX - 4),
                        positionX, y + 3, MUTED_TEXT, true);
            }
        }
    ...
    private String fitText(String text, int width) {
        if (text == null) return "";
        if (width <= 8 || this.font.width(text) <= width) return text;
        String ellipsis = "...";
        int target = Math.max(0, width - this.font.width(ellipsis));
        int end = text.length();
        while (end > 0 && this.font.width(text.substring(0, end)) > target) end--;
        return text.substring(0, end) + ellipsis;
    }

    private static String displayJob(String token) {
        if (token == null || token.isBlank()) return "Unassigned";
        try {
    ...
    private static String displayTool(String token) {
        if (token == null || token.isBlank()) return "Hand";
        return readable(token);
    }

    private static int toolColour(String token) {
        if (token == null) return MUTED_TEXT;
        return switch (token) {
    ...
    private static int statusColour(String status) {
        ...
        if (lower.contains("waiting") || lower.contains("hungry") || lower.contains("replanning")) return WARNING_TEXT;
        if (lower.contains("escaping") || lower.contains("wrong profession")) return BAD_TEXT;
        if (lower.contains("mining") || lower.contains("farming") || lower.contains("building")
                || lower.contains("researching") || lower.contains("operating")) return GOOD_TEXT;
        return TEXT;
    }
