...
public final class DiplomacyScreen extends StrategyScreen {
			...
			if (target.isActive()) {
				...
				if (war != null) {
					this.addRenderableWidget(Button
							.builder(Component.literal("Peace Conf."),
									button -> this.minecraft.setScreen(new PeaceConferenceScreen(this, targetId)))
	...
	protected void renderCustom(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
		...
		graphics.text(this.font, player.getName() + " | Political Power: " + (int) player.getPoliticalPower()
				+ " | Improve relations costs 10 PP; declaring war costs 25 PP.", 18, 56, MUTED_TEXT, true);
		...
		for (Civilisation target : otherCivilisations(player)) {
			...
			String state = war != null ? "AT WAR, score " + (int) Math.round(war.scoreFor(player.getId()))
					: target.isPuppet() ? "PUPPET OF " + target.getOverlordCivilisationId()
			...
			graphics.text(this.font, target.getName() + " [" + target.getId() + "]  " + state, 22, y,
					target.isActive() ? TEXT : MUTED_TEXT, true);
			graphics.text(this.font, "Relation " + relation, Math.max(250, this.width / 2), y, relationColour, true);
			y += 24;
		}
	...
	private static List<Civilisation> otherCivilisations(Civilisation player) {
		return StrategyClientContext.civilisations().stream().filter(Civilisation::isActive)
