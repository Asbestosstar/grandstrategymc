...
/**
 * Data-driven research screen showing historical dates, progress, catch-up
 * dates and unlocks.
 */
public final class ResearchScreen extends StrategyScreen {
	...
	protected void init() {
		...
		for (Technology technology : visible(civilisation)) {
			...
			String label = civilisation.hasTechnology(technology.getId()) ? "Researched"
	...
	private static List<Technology> visible(Civilisation civilisation) {
		return StrategyClientContext.technologies().stream().sorted(Comparator.comparingLong(Technology::getBaseYear))
				.toList();
	}
	...
	protected void renderCustom(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
		...
		for (Technology technology : visible(civilisation)) {
			...
			if (civilisation.hasTechnology(technology.getId())) {
				...
				state = String.format(Locale.ROOT, "ACTIVE %.2f%% | ~%s remaining", fraction * 100.0,
						formatDuration(remaining));
			}
			graphics.text(this.font,
					technology.getName() + " | base " + year(technology.getBaseYear()) + " | catch-up "
							+ year(technology.getBackwaterYear()) + (state.isBlank() ? "" : " | " + state),
