 ...
 *
 * The map uses actual Minecraft Overworld X/Z coordinates. Only chunks recorded
 * by WorldMapTracker are rendered; unexplored Minecraft terrain does not
 * appear.
 */
public class MapScreen extends StrategyScreen {
	...
	/**
	 * Null, FARM or FACTORY while the next/continued map clicks designate
	 * districts.
	 */
	...
	/**
	 * Opens the map directly in a district-placement mode, used by the factory
	 * manager.
	 */
	...
	private MapScreen(Screen parent, double centreBlockX, double centreBlockZ, double blocksPerPixel,
			boolean viewInitialised, String placementType, String statusMessage) {
		super("Grand Strategy", parent);
	...
	protected void init() {
		...
		this.addRenderableWidget(Button
				.builder(Component.literal(pp),
						button -> this.minecraft
								.setScreen(new EconomyScreen(this, StrategyClientContext.currentPlayerCountry())))
				.bounds(x, y, 70, 20).build());
		x += 72;

		String pop = playerCountry == null ? "Pop --" : "Pop " + playerCountry.getPopulation();
		this.addRenderableWidget(Button
				.builder(Component.literal(pop),
						button -> this.minecraft
								.setScreen(new EconomyScreen(this, StrategyClientContext.currentPlayerCountry())))
		...
		if (playerCountry == null) {
			...
			this.addRenderableWidget(Button.builder(Component.literal("Create Country: " + ign), button -> {
				...
				statusMessage = queued ? "Creating country at your server-authoritative Minecraft position..."
			...
			addIconButton(UiIcon.ECONOMY, "Economy & population", navX, navY,
					button -> this.minecraft.setScreen(new EconomyScreen(this, playerCountry)));
			navX += 24;
		}

		addIconButton(UiIcon.DIPLOMACY, "Diplomacy", navX, navY,
				button -> this.minecraft.setScreen(new DiplomacyScreen(this)));
		navX += 24;

		String spiritsTip = playerCountry == null ? "National spirits (0)"
				: "National spirits (" + playerCountry.getNationalSpiritIds().size() + ")";
		addIconButton(UiIcon.SPIRITS, spiritsTip, navX, navY,
				button -> this.minecraft.setScreen(new NationalSpiritsScreen(this)));
		navX += 24;

		addIconButton(UiIcon.FOCUS, "National focus tree", navX, navY,
				button -> this.minecraft.setScreen(new FocusTreeScreen(this)));
		navX += 24;

		boolean hasPendingEvent = playerCountry != null && playerCountry.hasPendingEvent();
		String eventTip = hasPendingEvent ? "Events - decision waiting" : "Events & decisions";
		addIconButton(UiIcon.EVENTS, eventTip, navX, navY, 20, 20, hasPendingEvent ? WARNING_TEXT : 0xFFD7DEE7,
				hasPendingEvent ? 0xFFFFFFB0 : 0xFFFFFFFF, hasPendingEvent ? 0xFFE0B422 : 0,
				button -> this.minecraft.setScreen(new EventsScreen(this)));
		navX += 24;

		if (playerCountry != null) {
			addIconButton(UiIcon.FARM_ZONE, "Designate farm district - click owned or uncolonised land", navX, navY,
					button -> {
						awaitingSoldierDestination = false;
						workZonePlacementType = "FARM".equals(workZonePlacementType) ? null : "FARM";
						statusMessage = workZonePlacementType == null ? "Farm-zone placement cancelled."
								: "Farm-zone mode: click owned or uncolonised discovered land; right-click to finish.";
					});
			navX += 24;
			addIconButton(UiIcon.FACTORY_ZONE, "Factories, districts & production", navX, navY,
					button -> this.minecraft.setScreen(new FactoryScreen(this)));
			navX += 24;
		}
		...
		if (playerCountry != null) {
			...
			addIconButton(UiIcon.ARMY_AUTO, "Army automatic control", 6, armyY, button -> {
				...
				statusMessage = queued ? "Army switched to automatic wartime movement and combat."
						: "Not connected to a Grand Strategy server.";
				if (queued)
					scheduleRefresh();
			});
		...
		addIconButton(UiIcon.ZOOM_OUT, "Zoom out", controlsX, navY, button -> zoomAtMapCentre(1.35));
		controlsX += 24;
		addIconButton(UiIcon.ZOOM_IN, "Zoom in", controlsX, navY, button -> zoomAtMapCentre(1.0 / 1.35));
		controlsX += 24;
		addIconButton(UiIcon.PLAYER, "Centre on player", controlsX, navY, button -> centreOnPlayer());
		controlsX += 24;
		addIconButton(UiIcon.FIT, "Fit discovered map", controlsX, navY, button -> fitToDiscovered());
	}
	...
	private void fitToDiscovered() {
		...
		blocksPerPixel = clamp(Math.max(spanX / usableWidth, spanZ / usableHeight) * 1.08, MIN_BLOCKS_PER_PIXEL,
				MAX_BLOCKS_PER_PIXEL);
		viewInitialised = true;
	}
	...
	protected void renderCustom(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
		...
		graphics.text(this.font, "Grand Strategy | " + year + " | " + formatRate(gsRate) + " | " + countryName, 18, 80,
				TEXT, true);
		if (player != null && player.hasPendingEvent()) {
			graphics.text(this.font, "EVENT DECISION WAITING - open Events to choose an outcome", 18, 91, WARNING_TEXT,
					true);
		}
		...
		int infoY = mapY + 5;
		String discovery = snapshot.isEmpty() ? "No Overworld land discovered yet"
				: "Discovered " + snapshot.tiles().size() + " chunks";
		graphics.text(this.font,
				discovery + " | centre X=" + (int) centreBlockX + " Z=" + (int) centreBlockZ + " | " + formatScale(),
		...
		if (!statusMessage.isBlank()) {
			graphics.text(this.font, statusMessage, 220, 58,
					statusMessage.toLowerCase(Locale.ROOT).contains("not running") ? BAD_TEXT : WARNING_TEXT, true);
		}
	}
	...
	private void renderDiscoveredTerrain(GuiGraphicsExtractor graphics, WorldMapTracker.Snapshot snapshot) {
		...
		for (WorldMapTracker.MapTile tile : snapshot.tiles()) {
			...
			if (x2 <= x1)
				x2 = Math.min(mapX + mapWidth, x1 + 1);
			if (y2 <= y1)
				y2 = Math.min(mapY + mapHeight, y1 + 1);
	...
	private void renderProvidenceTerritories(GuiGraphicsExtractor graphics, WorldMapTracker.Snapshot snapshot) {
		...
		for (Providence providence : StrategyClientContext.providences()) {
			if (providence == null || !providence.isEstablished())
				continue;
			for (long key : providence.getTerritoryChunkKeys())
				byChunk.put(key, providence);
		}
		...
		for (Map.Entry<Long, Providence> entry : byChunk.entrySet()) {
			...
			if (x2 < mapX || x1 > mapX + mapWidth || y2 < mapY || y1 > mapY + mapHeight)
				continue;
			...
			drawTerritoryBorderSide(graphics, byChunk, providence, controllerId, chunkX, chunkZ, -1, 0, x1, y1, x1 + 2,
					y2);
			drawTerritoryBorderSide(graphics, byChunk, providence, controllerId, chunkX, chunkZ, 1, 0, x2 - 2, y1, x2,
					y2);
			drawTerritoryBorderSide(graphics, byChunk, providence, controllerId, chunkX, chunkZ, 0, -1, x1, y1, x2,
					y1 + 2);
			drawTerritoryBorderSide(graphics, byChunk, providence, controllerId, chunkX, chunkZ, 0, 1, x1, y2 - 2, x2,
					y2);
		}
	}

	private void drawTerritoryBorderSide(GuiGraphicsExtractor graphics, Map<Long, Providence> byChunk,
			Providence providence, String controllerId, int chunkX, int chunkZ, int dx, int dz, int x1, int y1, int x2,
			int y2) {
	...
	private void renderProvidenceCities(GuiGraphicsExtractor graphics, WorldMapTracker.Snapshot snapshot) {
		for (Providence providence : StrategyClientContext.providences()) {
			...
			if (!snapshot.isDiscoveredBlock(city.getBlockX(), city.getBlockZ()))
				continue;
	...
	private void renderWorkZones(GuiGraphicsExtractor graphics, Civilisation player) {
		...
		for (PhysicalVillagerSystem.WorkZoneMapMarker zone : StrategyClientContext.workZones()) {
			if (zone == null || !Objects.equals(player.getId(), zone.civilisationId()))
				continue;
			...
			y2 = clampInt(y2, mapY, mapY + mapHeight);
			if (x2 <= x1)
				x2 = Math.min(mapX + mapWidth, x1 + 2);
			if (y2 <= y1)
				y2 = Math.min(mapY + mapHeight, y1 + 2);
	...
	private void renderWorkZonePlacementPreview(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
		if (workZonePlacementType == null || !insideMap(mouseX, mouseY))
			return;
	...
	private void renderVillagers(GuiGraphicsExtractor graphics, WorldMapTracker.Snapshot snapshot) {
		...
		for (PhysicalVillagerSystem.VillagerMapMarker marker : StrategyClientContext.villagers()) {
			if (marker == null || !snapshot.isDiscoveredBlock(marker.blockX(), marker.blockZ()))
				continue;
			int projectedX = worldToScreenX(marker.blockX());
			int projectedY = worldToScreenY(marker.blockZ());
			if (!insideMap(projectedX, projectedY))
				continue;
			...
			if (soldier)
				graphics.outline(x - 3, y - 4, 7, 9, 0xFFFFFFFF);
		}
	}

	/**
	 * Square-spiral offsets keep people projected onto the same map pixel
	...
	private int variantMarkerColour(int baseColour, int variant) {
		int[][] tweaks = new int[][] { { 0, 0, 0 }, { 20, 8, -10 }, { -12, 20, 10 }, { 18, -8, 22 }, { -20, 10, 24 },
				{ 24, 24, -12 }, { -8, -8, 28 } };
	...
	private void renderCountries(GuiGraphicsExtractor graphics, WorldMapTracker.Snapshot snapshot,
		...
		List<Civilisation> activeCivilisations = StrategyClientContext.civilisations().stream()
				.filter(Civilisation::isActive).filter(Civilisation::hasWorldMapPosition)
				.sorted(Comparator.comparing(Civilisation::getName, String.CASE_INSENSITIVE_ORDER)).toList();
		...
		for (Civilisation civilisation : activeCivilisations) {
			...
			countryMarkers.add(new MarkerBounds(cx - 5, cy - 6, Math.min(mapX + mapWidth, cx + 8 + labelWidth), cy + 8,
					civilisation.getId()));
		}
	}
	...
	private void renderArmyOrder(GuiGraphicsExtractor graphics, WorldMapTracker.Snapshot snapshot,
			Civilisation player) {
		if (player == null || !player.hasSoldierOrder())
			return;
	...
	private boolean clientCanDesignateZone(String civilisationId, String type, int blockX, int blockZ) {
		if (civilisationId == null || type == null)
			return false;
	...
	public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
		...
		if (event.button() == 0) {
			...
			if (awaitingSoldierDestination && insideMap(event.x(), event.y())) {
				...
				statusMessage = queued ? "Manual army order sent to X=" + worldX + " Z=" + worldZ + "."
	...
	protected StrategyScreen recreate() {
		return new MapScreen(getParentScreen(), centreBlockX, centreBlockZ, blocksPerPixel, viewInitialised,
				workZonePlacementType, statusMessage);
	}
