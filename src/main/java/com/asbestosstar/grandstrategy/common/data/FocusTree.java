...
/** Data-driven national focus tree. Completion state belongs to each civilisation. */
public class FocusTree {
    ...
    public String getCivilisationId() { return civilisationId; }
    public List<FocusNode> getNodes() { return nodes == null ? List.of() : List.copyOf(nodes); }

    public FocusNode getNode(String id) {
        if (id == null || nodes == null) return null;
        for (FocusNode node : nodes) {
            if (node != null && id.equals(node.id)) return node;
        }
        return null;
    }
    ...
    public static class FocusNode {
        ...
        @Deprecated private boolean completed;
        ...
        public FocusNode(String id, String title, String description,
                         int durationSteps, double politicalPowerCost, double aiWeight,
                         List<String> prerequisites, List<String> mutuallyExclusive,
        ...
        public List<String> getMutuallyExclusive() { return mutuallyExclusive == null ? List.of() : List.copyOf(mutuallyExclusive); }
        public int getDurationSteps() { return Math.max(1, durationSteps); }
        public double getPoliticalPowerCost() { return Math.max(0.0, politicalPowerCost); }
        ...
        @Deprecated public boolean isCompleted() { return completed; }
        @Deprecated public void setCompleted(boolean completed) { this.completed = completed; }
    }
}
