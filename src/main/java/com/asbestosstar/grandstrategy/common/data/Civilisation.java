...
public class Civilisation {
    ...
    // Persistent diplomacy state. These collections intentionally use only civilisation
    // ids / simple DTOs so every loader and multiplayer client can serialise them without
    // linking to another mod or loader-specific diplomacy API.
    private List<String> guaranteedCivilisationIds = new ArrayList<>();
    private List<String> allianceCivilisationIds = new ArrayList<>();
    private List<String> defensivePactCivilisationIds = new ArrayList<>();
    /** Countries to which THIS country has granted military access. */
    private List<String> militaryAccessGrantedToIds = new ArrayList<>();
    private List<String> researchAgreementCivilisationIds = new ArrayList<>();
    private List<String> royalMarriageCivilisationIds = new ArrayList<>();
    private String factionId;
    private Map<String, DiplomaticWarGoal> justifiedWarGoals = new LinkedHashMap<>();
    /** One incoming offer per source country: source id -> action id. */
    private Map<String, String> pendingDiplomaticOffers = new LinkedHashMap<>();
    private int diplomacyAiCooldownSteps;
    /** Abstract eligible royal children. A monarchy seeds a small house on first use. */
    private int royalChildren;
    ...
    public synchronized void normaliseAfterLoad() {
        ...
        if (guaranteedCivilisationIds == null) guaranteedCivilisationIds = new ArrayList<>();
        if (allianceCivilisationIds == null) allianceCivilisationIds = new ArrayList<>();
        if (defensivePactCivilisationIds == null) defensivePactCivilisationIds = new ArrayList<>();
        if (militaryAccessGrantedToIds == null) militaryAccessGrantedToIds = new ArrayList<>();
        if (researchAgreementCivilisationIds == null) researchAgreementCivilisationIds = new ArrayList<>();
        if (royalMarriageCivilisationIds == null) royalMarriageCivilisationIds = new ArrayList<>();
        if (justifiedWarGoals == null) justifiedWarGoals = new LinkedHashMap<>();
        if (pendingDiplomaticOffers == null) pendingDiplomaticOffers = new LinkedHashMap<>();
        diplomacyAiCooldownSteps = Math.max(0, diplomacyAiCooldownSteps);
        royalChildren = Math.max(0, royalChildren);
    ...
    private static boolean containsId(List<String> ids, String otherId) {
        return otherId != null && ids != null && ids.contains(otherId);
    }

    private static boolean setId(List<String> ids, String otherId, boolean enabled) {
        if (ids == null || otherId == null || otherId.isBlank()) return false;
        if (enabled) {
            if (ids.contains(otherId)) return false;
            ids.add(otherId);
            return true;
        }
        return ids.remove(otherId);
    }

    public synchronized List<String> getGuaranteedCivilisationIds() { return List.copyOf(guaranteedCivilisationIds); }
    public synchronized boolean guarantees(String otherId) { return containsId(guaranteedCivilisationIds, otherId); }
    public synchronized boolean setGuarantee(String otherId, boolean enabled) {
        if (otherId == null || otherId.equals(id)) return false;
        return setId(guaranteedCivilisationIds, otherId, enabled);
    }

    public synchronized List<String> getAllianceCivilisationIds() { return List.copyOf(allianceCivilisationIds); }
    public synchronized boolean isAlliedWith(String otherId) { return containsId(allianceCivilisationIds, otherId); }
    public synchronized boolean setAlliance(String otherId, boolean enabled) {
        if (otherId == null || otherId.equals(id)) return false;
        return setId(allianceCivilisationIds, otherId, enabled);
    }

    public synchronized List<String> getDefensivePactCivilisationIds() { return List.copyOf(defensivePactCivilisationIds); }
    public synchronized boolean hasDefensivePactWith(String otherId) { return containsId(defensivePactCivilisationIds, otherId); }
    public synchronized boolean setDefensivePact(String otherId, boolean enabled) {
        if (otherId == null || otherId.equals(id)) return false;
        return setId(defensivePactCivilisationIds, otherId, enabled);
    }

    public synchronized List<String> getMilitaryAccessGrantedToIds() { return List.copyOf(militaryAccessGrantedToIds); }
    public synchronized boolean grantsMilitaryAccessTo(String otherId) { return containsId(militaryAccessGrantedToIds, otherId); }
    public synchronized boolean setMilitaryAccessGrantedTo(String otherId, boolean enabled) {
        if (otherId == null || otherId.equals(id)) return false;
        return setId(militaryAccessGrantedToIds, otherId, enabled);
    }

    public synchronized List<String> getResearchAgreementCivilisationIds() { return List.copyOf(researchAgreementCivilisationIds); }
    public synchronized boolean hasResearchAgreementWith(String otherId) { return containsId(researchAgreementCivilisationIds, otherId); }
    public synchronized boolean setResearchAgreement(String otherId, boolean enabled) {
        if (otherId == null || otherId.equals(id)) return false;
        return setId(researchAgreementCivilisationIds, otherId, enabled);
    }

    public synchronized List<String> getRoyalMarriageCivilisationIds() { return List.copyOf(royalMarriageCivilisationIds); }
    public synchronized boolean hasRoyalMarriageWith(String otherId) { return containsId(royalMarriageCivilisationIds, otherId); }
    public synchronized boolean setRoyalMarriage(String otherId, boolean enabled) {
        if (otherId == null || otherId.equals(id)) return false;
        return setId(royalMarriageCivilisationIds, otherId, enabled);
    }

    public synchronized String getFactionId() {
        return factionId == null || factionId.isBlank() ? null : factionId;
    }

    public synchronized void setFactionId(String value) {
        factionId = value == null || value.isBlank() ? null : value.trim();
    }

    public synchronized boolean isInFactionWith(String otherId) {
        if (otherId == null || getFactionId() == null) return false;
        Civilisation other = DataManager.getCivilisations().get(otherId);
        return other != null && getFactionId().equals(other.getFactionId());
    }

    public synchronized DiplomaticWarGoal getWarGoalAgainst(String targetId) {
        if (targetId == null || justifiedWarGoals == null) return null;
        DiplomaticWarGoal goal = justifiedWarGoals.get(targetId);
        return goal == null ? null : goal.copy();
    }

    public synchronized void setWarGoal(DiplomaticWarGoal goal) {
        if (goal == null || goal.getTargetCivilisationId() == null
                || goal.getTargetCivilisationId().equals(id)) return;
        justifiedWarGoals.put(goal.getTargetCivilisationId(), goal.copy());
    }

    public synchronized void clearWarGoalAgainst(String targetId) {
        if (targetId != null && justifiedWarGoals != null) justifiedWarGoals.remove(targetId);
    }

    public synchronized Map<String, DiplomaticWarGoal> getWarGoals() {
        Map<String, DiplomaticWarGoal> copy = new LinkedHashMap<>();
        for (Map.Entry<String, DiplomaticWarGoal> entry : justifiedWarGoals.entrySet()) {
            if (entry.getKey() != null && entry.getValue() != null) copy.put(entry.getKey(), entry.getValue().copy());
        }
        return Map.copyOf(copy);
    }

    public synchronized void setPendingDiplomaticOffer(String sourceId, String actionId) {
        if (sourceId == null || sourceId.equals(id) || actionId == null || actionId.isBlank()) return;
        pendingDiplomaticOffers.put(sourceId, actionId);
    }

    public synchronized String getPendingDiplomaticOfferFrom(String sourceId) {
        return sourceId == null ? null : pendingDiplomaticOffers.get(sourceId);
    }

    public synchronized Map<String, String> getPendingDiplomaticOffers() {
        return Map.copyOf(pendingDiplomaticOffers);
    }

    public synchronized void clearPendingDiplomaticOfferFrom(String sourceId) {
        if (sourceId != null) pendingDiplomaticOffers.remove(sourceId);
    }

    public synchronized int getDiplomacyAiCooldownSteps() { return Math.max(0, diplomacyAiCooldownSteps); }
    public synchronized void setDiplomacyAiCooldownSteps(int steps) { diplomacyAiCooldownSteps = Math.max(0, steps); }
    public synchronized void tickDiplomacyAiCooldown() { if (diplomacyAiCooldownSteps > 0) diplomacyAiCooldownSteps--; }

    public synchronized int getRoyalChildren() {
        if (isMonarchyGovernment() && royalChildren <= 0) {
            // Old saves did not have dynastic state. Seed a small deterministic royal
            // household so the royal-marriage mechanic is usable without a migration file.
            royalChildren = 1 + Math.floorMod((id == null ? 0 : id.hashCode()), 3);
        }
        return Math.max(0, royalChildren);
    }

    public synchronized boolean consumeRoyalChild() {
        if (getRoyalChildren() <= 0) return false;
        royalChildren--;
        return true;
    }

    public synchronized boolean isMonarchyGovernment() {
        Ideology definition = DataManager.findIdeology(ideology);
        String family = definition == null ? "" : String.valueOf(definition.getFamilyId()).toLowerCase(Locale.ROOT);
        String government = ideology == null ? "" : ideology.toLowerCase(Locale.ROOT);
        return family.contains("monarch") || government.contains("monarch") || government.contains("palatial");
    }
