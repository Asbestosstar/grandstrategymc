...
public class Civilisation {
	...
	/**
	 * State religion/policy. "secular" is valid here but not for ordinary
	 * people/leaders.
	 */
	...
	/**
	 * Country-specific names assigned to newly colonised cities, independent of
	 * country ownership.
	 */
	...
	// This prevents historical start-date logic from reactivating it on the next
	// tick.
	...
	// Stable Minecraft Overworld anchor once this civilisation has been placed on
	// discovered land.
	...
	// Physical army control. AUTO lets server-side soldiers seek wartime
	// objectives;
	...
	public Civilisation(String id, String name, String defaultLeaderId, String ideology, String religion,
			List<String> startingProvidences) {
		this(id, name, defaultLeaderId, ideology, religion, startingProvidences, Long.MAX_VALUE, null, null);
	}

	public Civilisation(String id, String name, String defaultLeaderId, String ideology, String religion,
			List<String> startingProvidences, long startYear, Integer mapXPercent, Integer mapYPercent) {
		this.id = id;
		this.name = name;
		...
		this.startingProvidences = startingProvidences == null ? new ArrayList<>()
				: new ArrayList<>(startingProvidences);
	...
	public synchronized void normaliseAfterLoad() {
		if (startingProvidences == null)
			startingProvidences = new ArrayList<>();
		if (populationReligions == null)
			populationReligions = new LinkedHashMap<>();
		if (ideologySupport == null)
			ideologySupport = new LinkedHashMap<>();
		if (defaultCityNames == null)
			defaultCityNames = new ArrayList<>();
		if (completedTechnologyIds == null)
			completedTechnologyIds = new ArrayList<>();
		if (productionQueue == null)
			productionQueue = new ArrayList<>();
		if (resources == null)
			resources = new LinkedHashMap<>();
		if (villagerJobs == null)
			villagerJobs = new LinkedHashMap<>();
		if (nationalSpiritIds == null)
			nationalSpiritIds = new ArrayList<>();
		if (diplomaticRelations == null)
			diplomaticRelations = new HashMap<>();
		if (completedFocusIds == null)
			completedFocusIds = new ArrayList<>();
		if (eventHistory == null)
			eventHistory = new ArrayList<>();
		if (ideology == null || ideology.isBlank())
			ideology = "independent";
		if (defaultGovernment == null || defaultGovernment.isBlank())
			defaultGovernment = ideology;
		if (religion == null || religion.isBlank() || "neutral".equalsIgnoreCase(religion))
			religion = "secular";
		...
		if (baseBorderColourArgb == null || (baseBorderColourArgb >>> 24) == 0) {
			...
			baseBorderColourArgb = borderColourArgb != null && (borderColourArgb >>> 24) != 0 ? opaque(borderColourArgb)
	...
	/**
	 * Starts a new player country with the required initial population of 25
	 * villagers.
	 */
	...
	/**
	 * Used by future liberation/restoration mechanics: later restorations begin
	 * larger.
	 */
	...
	/**
	 * Historical population scaling: roughly doubles every 2,500 years after 4000
	 * BCE.
	 */
	...
	/**
	 * Rebuilds all job assignments while preserving population and conscription
	 * level.
	 */
	...
	public synchronized boolean reassignOneFrom(VillagerJob source) {
		...
		VillagerJob target = source == VillagerJob.FARMER ? VillagerJob.ADMINISTRATOR : VillagerJob.FARMER;
		villagerJobs.put(source.name(), getJobCount(source) - 1);
		villagerJobs.put(target.name(), getJobCount(target) + 1);
		return true;
	}
	...
	private VillagerJob largestCivilianJobExcept(VillagerJob excluded) {
		...
		int bestCount = -1;
		for (VillagerJob job : VillagerJob.values()) {
			if (job == VillagerJob.SOLDIER || job == excluded)
				continue;
	...
	private int desiredSoldierCount() {
		return Math.max(0,
				Math.min(population, (int) Math.round(population * getConscriptionLevel().getSoldierShare())));
	}
	...
	public synchronized void addPopulationRandomised(int amount) {
		...
		List<VillagerJob> civilianJobs = new ArrayList<>(
				List.of(VillagerJob.FARMER, VillagerJob.LUMBERJACK, VillagerJob.MINER, VillagerJob.FACTORY_BUILDER,
						VillagerJob.ROAD_BUILDER, VillagerJob.RESEARCHER, VillagerJob.ADMINISTRATOR));
	 ...
	 * settlement-supporting weights. Conscription may subsequently move the
	 * required share into the soldier bucket.
	 */
	...
	private VillagerJob randomCivilianJob() {
		...
		if (roll < 0.37)
			return VillagerJob.LUMBERJACK;
		if (roll < 0.57)
			return VillagerJob.MINER;
		if (roll < 0.72)
			return VillagerJob.FACTORY_BUILDER;
		if (roll < 0.83)
			return VillagerJob.ROAD_BUILDER;
		if (roll < 0.92)
			return VillagerJob.RESEARCHER;
	...
	/**
	 * Exact population reset used only when an existing society splits in a civil
	 * war.
	 */
	...
	/**
	 * Removes a physical casualty from its actual job bucket before conscription is
	 * rebalanced.
	 */
	...
	public synchronized Map<VillagerJob, Integer> getJobSnapshot() {
		...
		for (VillagerJob job : VillagerJob.values())
			snapshot.put(job, getJobCount(job));
	...
	public synchronized Map<ResourceType, Double> getResourceSnapshot() {
		...
		for (ResourceType type : ResourceType.values())
			snapshot.put(type, getResource(type));
		return Collections.unmodifiableMap(snapshot);
	}
	...
	public synchronized boolean spendPoliticalPower(double amount) {
		if (amount <= 0.0)
			return true;
		if (politicalPower + 1.0e-9 < amount)
			return false;
	...
	public synchronized boolean completeFactoryIfReady() {
		if (factoryConstructionProgress < 100.0)
			return false;
		Map<ResourceType, Double> costs = Map.of(ResourceType.WOOD, 80.0, ResourceType.STONE, 120.0, ResourceType.IRON,
				40.0);
	...
	public synchronized boolean completeRoadIfReady() {
		if (roadConstructionProgress < 100.0)
			return false;
	...
	public synchronized int consumeWholeBirths() {
		...
		if (births > 0)
			populationGrowthAccumulator -= births;
		return births;
	}
	...
	public synchronized String getActiveTechnologyId() {
		return activeTechnologyId;
	}

	public synchronized double getActiveTechnologyProgress() {
		return Math.max(0.0, activeTechnologyProgress);
	}

	public synchronized boolean startTechnology(String technologyId) {
		if (technologyId == null || technologyId.isBlank() || hasTechnology(technologyId))
			return false;
		if (technologyId.equals(activeTechnologyId))
			return true;
	...
	public synchronized void addTechnologyProgress(double amount) {
		if (activeTechnologyId == null || amount <= 0.0)
			return;
	...
	public synchronized boolean completeTechnology(String technologyId) {
		if (technologyId == null || technologyId.isBlank())
			return false;
		if (completedTechnologyIds == null)
			completedTechnologyIds = new ArrayList<>();
	...
	public synchronized ProductionOrder queueProduction(String recipeId, int amount) {
		if (recipeId == null || recipeId.isBlank() || amount <= 0)
			return null;
		if (productionQueue == null)
			productionQueue = new ArrayList<>();
	...
	public synchronized boolean cancelProduction(long serial) {
		return productionQueue != null
				&& productionQueue.removeIf(order -> order != null && order.getSerial() == serial);
	}

	public synchronized ProductionOrder firstProductionOrderForRecipeIds(List<String> allowedRecipeIds) {
		if (productionQueue == null || allowedRecipeIds == null || allowedRecipeIds.isEmpty())
			return null;
		productionQueue.removeIf(order -> order == null || order.isComplete());
		for (ProductionOrder order : productionQueue) {
			if (!order.isPaused() && order.getRemaining() > 0 && allowedRecipeIds.contains(order.getRecipeId()))
				return order;
		}
		return null;
	}
	...
	public synchronized void recordProduction(long serial, int amount) {
		if (productionQueue == null || amount <= 0)
			return;
		for (ProductionOrder order : productionQueue) {
	...
	public synchronized boolean addNationalSpiritId(String spiritId) {
		if (NationalSpirit.byId(spiritId) == null || nationalSpiritIds.contains(spiritId))
			return false;
	...
	public synchronized String getActiveFocusId() {
		return activeFocusId;
	}

	public synchronized double getActiveFocusProgress() {
		return Math.max(0.0, activeFocusProgress);
	}

	public synchronized List<String> getCompletedFocusIds() {
		return List.copyOf(completedFocusIds);
	}
	...
	public synchronized boolean beginFocus(String focusId, double politicalPowerCost) {
		if (!active || focusId == null || focusId.isBlank() || activeFocusId != null || hasCompletedFocus(focusId))
			return false;
		if (!spendPoliticalPower(Math.max(0.0, politicalPowerCost)))
			return false;
	...
	public synchronized double advanceActiveFocus(double amount) {
		if (activeFocusId == null || amount <= 0.0)
	...
	public synchronized boolean completeFocus(String focusId) {
		if (focusId == null || !focusId.equals(activeFocusId))
			return false;
		if (!completedFocusIds.contains(focusId))
			completedFocusIds.add(focusId);
		activeFocusId = null;
		activeFocusProgress = 0.0;
		return true;
	}
	...
	public synchronized String getPendingEventId() {
		return pendingEventId;
	}

	public synchronized boolean hasPendingEvent() {
		return pendingEventId != null && !pendingEventId.isBlank();
	}

	public synchronized int getEventCooldownSteps() {
		return Math.max(0, eventCooldownSteps);
	}

	public synchronized long getEventSerial() {
		return Math.max(0L, eventSerial);
	}
	...
	public synchronized boolean queueEvent(String eventId) {
		if (!active || hasPendingEvent() || eventId == null || eventId.isBlank())
			return false;
	...
	public synchronized void tickEventCooldown() {
		if (eventCooldownSteps > 0)
			eventCooldownSteps--;
	}

	public synchronized int getRelation(String otherCivilisationId) {
		return Math.max(-100, Math.min(100, diplomaticRelations.getOrDefault(otherCivilisationId, 0)));
	}

	public synchronized void modifyRelation(String otherCivilisationId, int delta) {
		if (otherCivilisationId == null || otherCivilisationId.equals(id))
			return;
	...
	public String getDefaultGovernment() {
		return defaultGovernment;
	}
	...
	public synchronized String getStateReligionId() {
		return religion;
	}

	public synchronized String getSnapStateReligionId() {
		return snapStateReligionId;
	}

	public synchronized void setStateReligionId(String religionId) {
		if (religionId != null && !religionId.isBlank())
			religion = religionId;
	}

	public synchronized void setSnapStateReligionId(String religionId) {
		snapStateReligionId = religionId;
	}

	public synchronized double getReligiousExtremism() {
		return clamp(religiousExtremism, 0.0, 100.0);
	}

	public synchronized void setReligiousExtremism(double value) {
		religiousExtremism = clamp(value, 0.0, 100.0);
	}

	public synchronized void addReligiousExtremism(double delta) {
		setReligiousExtremism(religiousExtremism + delta);
	}

	public synchronized Map<String, Double> getPopulationReligions() {
		return Map.copyOf(populationReligions);
	}
	...
	public synchronized String getPopulationPluralityReligion() {
		return populationReligions.entrySet().stream().max(Map.Entry.comparingByValue()).map(Map.Entry::getKey)
				.orElse("irreligion");
	}
	...
	public synchronized void shiftPopulationReligionToward(String religionId, double amount) {
		if (religionId == null || religionId.isBlank() || amount <= 0.0)
			return;
		normalisePopulationReligionShares();
	...
	public synchronized double getReligiousPopulationShare() {
		...
		for (Map.Entry<String, Double> entry : populationReligions.entrySet()) {
			...
			if ("atheism".equalsIgnoreCase(key) || "irreligion".equalsIgnoreCase(key)
					|| "secular".equalsIgnoreCase(key))
				continue;
	...
	/**
	 * Moves population share toward irreligion (negative) or the state religion
	 * (positive).
	 */
	public synchronized void adjustPopulationReligiosity(double delta) {
		...
		if (delta < 0.0) {
			...
			double source = populationReligions.getOrDefault("irreligion", 0.0)
					+ populationReligions.getOrDefault("atheism", 0.0);
			double take = Math.min(source, change);
			if (take > 0.0) {
				double fromIrreligion = Math.min(take, populationReligions.getOrDefault("irreligion", 0.0));
				populationReligions.put("irreligion",
						Math.max(0.0, populationReligions.getOrDefault("irreligion", 0.0) - fromIrreligion));
				double rest = take - fromIrreligion;
				if (rest > 0)
					populationReligions.put("atheism",
							Math.max(0.0, populationReligions.getOrDefault("atheism", 0.0) - rest));
	...
	public synchronized Map<String, Double> getIdeologySupport() {
		return Map.copyOf(ideologySupport);
	}

	public synchronized double getIdeologicalExtremism() {
		return clamp(ideologicalExtremism, 0.0, 100.0);
	}

	public synchronized void setIdeologicalExtremism(double value) {
		ideologicalExtremism = clamp(value, 0.0, 100.0);
	}

	public synchronized void addIdeologicalExtremism(double delta) {
		setIdeologicalExtremism(ideologicalExtremism + delta);
	}

	public synchronized String getPopulationPluralityIdeology() {
		return ideologySupport.entrySet().stream().max(Map.Entry.comparingByValue()).map(Map.Entry::getKey)
				.orElse("nonaligned");
	}

	public synchronized void setIdeologySupport(String ideologyId, double share) {
		if (ideologyId == null || ideologyId.isBlank())
			return;
	...
	public synchronized void shiftIdeologySupportToward(String ideologyId, double amount) {
		if (ideologyId == null || ideologyId.isBlank() || amount <= 0.0)
			return;
		normaliseIdeologyShares();
	...
	public synchronized String getSnapIdeologyId() {
		return snapIdeologyId;
	}

	public synchronized void setSnapIdeologyId(String ideologyId) {
		snapIdeologyId = ideologyId;
	}

	public synchronized double getStartingPopulationModifier() {
		return startingPopulationModifier;
	}
	...
	public synchronized List<String> getDefaultCityNames() {
		return List.copyOf(defaultCityNames);
	}

	public synchronized void setDefaultCityNames(List<String> names) {
		defaultCityNames = names == null ? new ArrayList<>()
				: new ArrayList<>(names.stream().filter(name -> name != null && !name.isBlank()).toList());
	...
	public synchronized String nextDefaultCityName() {
		if (defaultCityNames == null || defaultCityNames.isEmpty())
			return name;
	...
	public synchronized int getCivilWarCooldownSteps() {
		return civilWarCooldownSteps;
	}

	public synchronized void setCivilWarCooldownSteps(int steps) {
		civilWarCooldownSteps = Math.max(0, steps);
	}

	public synchronized void tickCivilWarCooldown() {
		if (civilWarCooldownSteps > 0)
			civilWarCooldownSteps--;
	}

	public List<String> getStartingProvidences() {
		return startingProvidences == null ? List.of() : List.copyOf(startingProvidences);
	}
	...
	public synchronized void addStartingProvidenceId(String providenceId) {
		...
		if (startingProvidences == null)
			startingProvidences = new ArrayList<>();
		if (!startingProvidences.contains(providenceId))
			startingProvidences.add(providenceId);
	}
	...
	public synchronized void redefineDefaultCityNames(List<String> cityNames) {
		...
		if (nextDefaultCityNameIndex < 0)
			nextDefaultCityNameIndex = 0;
	}

	/** Simple testable government transition used by the Politics screen. */
	public synchronized void cycleGovernment() {
		List<String> governments = List.of("Independent", "Council Republic", "Monarchy", "Theocracy", "Oligarchy",
				"Technocracy", "Republic");
	 ...
	 * only for the first real active -> collapsed transition so callers can log the
	 * event exactly once.
	 */
	...
	public boolean hasEstablishedHomeland() {
		return homelandEstablished;
	}
	...
	public double getPoliticalPower() {
		return Math.max(0.0, politicalPower);
	}

	public int getFactories() {
		return Math.max(0, factories);
	}

	public int getRoadSegments() {
		return Math.max(0, roadSegments);
	}

	public double getFactoryConstructionProgress() {
		return Math.max(0.0, factoryConstructionProgress);
	}

	public double getRoadConstructionProgress() {
		return Math.max(0.0, roadConstructionProgress);
	}

	public double getPopulationGrowthAccumulator() {
		return Math.max(0.0, populationGrowthAccumulator);
	}

	public double getResearchPoints() {
		return Math.max(0.0, researchPoints);
	}

	public int getMapXPercent() {
		if (mapXPercent != null)
			return clampInt(mapXPercent, 5, 95);
		return 8 + Math.floorMod(id == null ? 0 : id.hashCode(), 84);
	}

	public int getMapYPercent() {
		if (mapYPercent != null)
			return clampInt(mapYPercent, 8, 92);
	...
	private void refreshGovernmentColour() {
		int base = baseBorderColourArgb == null ? defaultBorderColourForId(id) : opaque(baseBorderColourArgb);
		borderColourArgb = colourForGovernment(base, defaultGovernment, ideology);
		borderColourGovernment = ideology;
	}

	/**
	 * Built-ins have deliberately separated colours; custom/player countries use a
	 * vivid ID-derived hue.
	 */
	...
	public static boolean hasBuiltInDefaultColour(String id) {
		...
		case "uruk", "eridu", "susa", "ur", "egypt", "indus", "minoan", "caral", "cliosoffice" -> true;
		default -> false;
		};
	}
	...
	private static int generatedVividColour(String key) {
		...
		return 0xFF000000 | ((int) Math.round(r * 255.0) << 16) | ((int) Math.round(g * 255.0) << 8)
				| (int) Math.round(b * 255.0);
	}

	private static int colourForGovernment(int base, String defaultGovernment, String government) {
		base = opaque(base);
		if (sameGovernment(defaultGovernment, government))
			return base;
	...
	private synchronized void normalisePopulationReligionShares() {
		if (populationReligions == null)
			populationReligions = new LinkedHashMap<>();
		populationReligions.entrySet().removeIf(entry -> entry.getKey() == null || entry.getKey().isBlank()
	...
	private synchronized void normaliseIdeologyShares() {
		if (ideologySupport == null)
			ideologySupport = new LinkedHashMap<>();
		ideologySupport.entrySet().removeIf(entry -> entry.getKey() == null || entry.getKey().isBlank()
				|| entry.getValue() == null || entry.getValue() <= 0.0);
