...
public final class MinecraftItemRegistry {
		...
		try {
			...
			for (Method method : itemRegistry.getClass().getMethods()) {
				...
				if (!method.getParameterTypes()[0].isAssignableFrom(item.getClass())
						&& !method.getParameterTypes()[0].isAssignableFrom(Item.class))
					continue;
				Object value = method.invoke(itemRegistry, item);
				if (value != null)
	...
	private static Item probeItem(String itemId) {
		try {
			...
			if (value == null) {
				try {
					...
					if (optionalValue instanceof Optional<?> result)
		...
		try {
			...
			if ("minecraft:blast_furnace".equals(itemId))
			...
			if ("minecraft:paper".equals(itemId))
				return Items.PAPER;
		} catch (Throwable ignored) {
		}
		return null;
