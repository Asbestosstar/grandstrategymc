...
public class GrandStrategyEvent {
	...
	public GrandStrategyEvent(String id, String title, String description, long minYear, long maxYear,
			double minStability, double maxStability, int minPopulation, double weight, List<String> civilisationIds,
	...
	public boolean canTrigger(Civilisation civilisation, long year) {
		...
		if (year < minYear || year > maxYear)
			return false;
		if (civilisation.getStability() < minStability || civilisation.getStability() > maxStability)
			return false;
		if (civilisation.getPopulation() < minPopulation)
			return false;
		return civilisationIds == null || civilisationIds.isEmpty() || civilisationIds.contains(civilisation.getId());
	}
	...
	public EventOption getOption(String optionId) {
		...
		for (EventOption option : options) {
			if (option != null && optionId.equals(option.id))
				return option;
		}
		return null;
	}
	...
	public static final class EventOption {
		...
		public EventOption(String id, String label, String description, double aiWeight, List<StrategyEffect> effects) {
			this.id = id;
			this.label = label;
			this.description = description;
